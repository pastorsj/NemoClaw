...
| Same-UID lifecycle control in the OpenShell-managed topology | The root controller authenticates the host action, verifies a stable process shape, pidfd-targets the observed child, and proves replacement health. It cannot establish malicious process provenance when the supervisor, gateway, and agent share the sandbox UID. Mutable config retains managed cold-start trust and time-of-check/time-of-use limits. | Use a direct root-entrypoint deployment when UID isolation and the transactional root restart seal are required. Do not treat the mutable config hash as a durable immutability anchor, and recreate a sandbox after suspected same-UID compromise. |
