...
<AccordionGroup>
  ...
  <Accordion title="Add Langfuse observability">
    ...
    For a self-hosted Langfuse deployment, copy `packages/nemoclaw-hermes/provider-profiles/langfuse-hermes-v1.yaml` to a new profile, change its `id` and exact `endpoints[0].host`, and import that reviewed profile before running `credentials add` with the new type.
