...
Stock onboarding selects a Pi managed-image digest. It does not build the Pi Dockerfile on the host. The reviewed Dockerfile is a trusted CI image-build source, not a user onboarding mechanism. A missing, incomplete, mutable, or inconsistent image contract fails before sandbox creation.
...
- [Pi Candidate Qualification and Security](../reference/pi-support) records the candidate qualification matrix, policy, credential, and qualification boundaries.
