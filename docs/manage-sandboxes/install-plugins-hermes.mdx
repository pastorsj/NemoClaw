...
The separated gateway never loads code from that sandbox-writable directory.
Instead, gateway startup installs the bundled Hindsight dependency into a gateway-owned runtime directory under `/run/nemoclaw` and loads it only from there.
...
When `memory.provider` is `hindsight`, gateway startup independently installs the supported client into `/run/nemoclaw/hermes-gateway-lazy-packages` as the gateway user before it launches.
The sandbox user cannot read or modify that gateway-owned tree, and the gateway does not activate the sandbox-owned tree.
Both directories are outside the generated `.env`, `config.yaml`, and `.config-hash` files.
The sandbox copy is part of the Hermes durable state plan; the gateway copy lasts for the container runtime and is recreated after a rebuild or container replacement.
...
Gateway startup always recreates its private runtime target when the container has been replaced and reinstalls the supported client as the gateway user when Hindsight is selected.
...
The one exception is the managed Hermes Dockerfile itself: passing the `packages/nemoclaw-hermes/Dockerfile` from the NemoClaw checkout the CLI runs from stages the repository root as the build context, exactly as the managed build does.
The managed exception applies the `.dockerignore` from the repository root, not one under `packages/nemoclaw-hermes/`.
...
<Warning>
  ...
  Your Dockerfile must also preserve the NemoClaw Hermes layers from `packages/nemoclaw-hermes/Dockerfile`.
</Warning>
