...
Each standard runtime layer is stored under `packages/nemoclaw-<id>/` with its agent manifest, image files, startup script, and baseline policy.
`$$nemoclaw harness install <harness>` copies one bundled package to `~/.nemoclaw/harnesses` without executing package code.
Onboarding uses the installed copy when present and keeps the existing bundled choices available.
The host CLI continues to own command parsing, credentials, OpenShell lifecycle operations, and shared messaging channels.

| Agent | Integration files | Runtime behavior |
|---|---|---|
<AgentOnly variant="openclaw">
| OpenClaw | `packages/nemoclaw-openclaw/plugin/openclaw.plugin.json`, `packages/nemoclaw-openclaw/plugin/src/runtime-context.ts`, and the TypeScript package under `packages/nemoclaw-openclaw/plugin/src/` | Registers the `/nemoclaw` slash command, adds the NemoClaw inference provider, and injects sandbox and policy context into OpenClaw turns. |
</AgentOnly>
<AgentOnly variant="hermes">
| Hermes | `packages/nemoclaw-hermes/manifest.yaml`, `packages/nemoclaw-hermes/plugin/plugin.yaml`, `packages/nemoclaw-hermes/generate-config.ts`, `packages/nemoclaw-hermes/config/`, and `packages/nemoclaw-hermes/start.sh` | Declares the Hermes agent contract, installs the NemoClaw Hermes plugin, writes `/sandbox/.hermes/config.yaml` and `/sandbox/.hermes/.env`, and launches `hermes gateway run` behind the OpenShell proxy. |
</AgentOnly>
<AgentOnly variant="deepagents">
| Deep Agents | `packages/nemoclaw-langchain-deepagents-code/manifest.yaml`, `packages/nemoclaw-langchain-deepagents-code/generate-config.ts`, `packages/nemoclaw-langchain-deepagents-code/start.sh`, and the managed `dcode` launchers | Declares the terminal agent contract, writes `/sandbox/.deepagents/config.toml`, installs managed wrappers for `dcode` and `dcode -n`, and routes inference through `inference.local`. |
</AgentOnly>

<AgentOnly variant="openclaw">
The OpenClaw integration is a thin TypeScript plugin that runs in-process with the OpenClaw gateway inside the sandbox.
Its durable entry points are `packages/nemoclaw-openclaw/plugin/src/index.ts`, `packages/nemoclaw-openclaw/plugin/src/runtime-context.ts`, and `packages/nemoclaw-openclaw/plugin/openclaw.plugin.json`.
The `packages/nemoclaw-openclaw/plugin/src/commands/` directory contains in-sandbox `/nemoclaw` command handlers and migration helpers.
The `packages/nemoclaw-openclaw/plugin/src/blueprint/` directory contains runner, state, snapshot, SSRF, and private-network validation code.
...
├── model-specific-setup/           Shared compatibility schema and registry guidance
...
<AgentOnly variant="openclaw">
The default OpenClaw policy starts from `packages/nemoclaw-openclaw/policy-additions.yaml`.
</AgentOnly>
<AgentOnly variant="hermes">
Hermes keeps its agent-owned image, plugin, config, entrypoint, and policy additions under `packages/nemoclaw-hermes/`.
The default Hermes policy starts from `packages/nemoclaw-hermes/policy-additions.yaml`.
</AgentOnly>
<AgentOnly variant="deepagents">
Deep Agents keeps its agent-owned image, config generator, entrypoint, wrappers, and policy additions under `packages/nemoclaw-langchain-deepagents-code/`.
The default Deep Agents policy starts from `packages/nemoclaw-langchain-deepagents-code/policy-additions.yaml`.
</AgentOnly>

The current blueprint runner implementation lives in the OpenClaw package's TypeScript plugin:
...
<AgentOnly variant="deepagents">
Deep Agents onboarding builds from the agent-specific `packages/nemoclaw-langchain-deepagents-code/Dockerfile.base` image and layers the managed Deep Agents runtime Dockerfile on top.
...
NemoClaw keeps those declarations under `packages/nemoclaw-<id>/model-specific-setup/<agent>/` so each agent runtime owns its compatibility behavior.
The shared schema and registry guidance remain under `nemoclaw-blueprint/model-specific-setup/`.
