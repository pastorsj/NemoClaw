...
`$$nemoclaw harness install <id>` copies one bundled package to `~/.nemoclaw/harnesses` without executing package code.
...
| Hermes | `packages/nemoclaw-hermes/manifest.yaml`, `packages/nemoclaw-hermes/plugin/plugin.yaml`, `packages/nemoclaw-hermes/config/generate-config.ts`, `packages/nemoclaw-hermes/config/`, and `packages/nemoclaw-hermes/start.sh` | Declares the Hermes agent contract, installs the NemoClaw Hermes plugin, writes `/sandbox/.hermes/config.yaml` and `/sandbox/.hermes/.env`, and launches `hermes gateway run` behind the OpenShell proxy. |
</AgentOnly>
<AgentOnly variant="deepagents">
| Deep Agents | `packages/nemoclaw-langchain-deepagents-code/manifest.yaml`, `packages/nemoclaw-langchain-deepagents-code/config/generate-config.ts`, `packages/nemoclaw-langchain-deepagents-code/start.sh`, and the managed `dcode` launchers | Declares the terminal agent contract, writes `/sandbox/.deepagents/config.toml`, installs managed wrappers for `dcode` and `dcode -n`, and routes inference through `inference.local`. |
</AgentOnly>
