...
If its diagnostics report manual cleanup, use only the printed container command.
...
Use Bash to enter and validate the path from the error before you remove its contents:
...
- If `policy list` reports no disagreement for the unconfirmed mutation, compare the live preset set with the selection in the failed session.
  ...
  Otherwise, start fresh onboarding and select the preset set that `policy list` reports as active.
...
You must remove the directory only by the path that NemoClaw reports.
You must confirm that the path no longer exists before you rerun the complete command.
</Warning>
...
Resumed onboarding and sandbox rebuilds warm the recorded Ollama model and repeat this check before reusing its route.
