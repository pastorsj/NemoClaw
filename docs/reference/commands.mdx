...
OpenClaw is the default agent for `$$nemoclaw onboard` unless you select another available agent runtime with `--agent <name>` or set `NEMOCLAW_AGENT=<name>`.
Run `$$nemoclaw agents list` to see the available agent runtime names; for example, use `hermes` for Hermes or `langchain-deepagents-code` for LangChain Deep Agents Code.
...
List the available agent runtimes that can be selected with `$$nemoclaw onboard --agent <name>`.
Use this global command when you need valid runtime names before creating or recreating a sandbox.
It lists runtime names with the descriptions from their agent manifests.
...
List the bundled and installed harness packages that NemoClaw can use during onboarding.
The output includes each harness ID, package name, version, and source.
This command does not change host or sandbox state.

```bash
$$nemoclaw harness list
```

The source is `bundled` for a package shipped with NemoClaw.
The source is `installed` for a user-scoped copy under `~/.nemoclaw/harnesses`.

### `$$nemoclaw harness install <harness>`

Install one bundled harness package into `~/.nemoclaw/harnesses`.
Bundled harnesses remain available before installation, so existing onboarding commands do not change.
The NemoClaw installer installs the selected package before it starts onboarding.
The install does not execute package code, create a sandbox, or request credentials.

<Warning>
Treat an installed agent runtime package as trusted code.
Later onboarding and lifecycle commands can give package-owned helpers access to NemoClaw credentials and sandbox control.
The installation receipt detects later content changes, but it does not authenticate the publisher or validate package behavior.
</Warning>
...
Use `--refresh-installed` to update every clean installed package that is bundled by the current NemoClaw build.
You can also install one package and refresh the other clean installed packages in the same command.

```bash
$$nemoclaw harness install --refresh-installed
$$nemoclaw harness install hermes --refresh-installed
```

The installation succeeds when the same package is already present.
The receipt tracks regular package files and executable bits, except entries named `.git`, `.DS_Store`, `node_modules`, or `__pycache__`.
Changes in those ignored entries do not prevent an upgrade and are removed when replacement succeeds.
...
NemoClaw replaces an installed copy when bundled content has changed only if the receipt-tracked content still matches its installation receipt.
If replacement fails before verification, NemoClaw attempts to restore the prior package.
If restoration also fails, NemoClaw stops and reports the retained prior-package path.
NemoClaw stops without replacement when receipt-tracked content has local changes.
After installation, `$$nemoclaw harness list` reports the selected harness source as `installed`.
...
Use `--agent <name>` to target a specific available agent runtime during onboarding.
The `$$nemoclaw onboard --help` output lists available runtime names inline, and `$$nemoclaw agents list` shows the same runtimes with manifest descriptions.
