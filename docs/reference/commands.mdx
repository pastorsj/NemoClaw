...
List the harness packages installed for the current user and the reviewed packages bundled with this NemoClaw build.
An installed row identifies a package whose stored receipt and content passed verification.
An available row identifies a reviewed package that NemoClaw can install; it does not mean that package is installed.

```bash
nemoclaw harness list [--json]
```

On a fresh installation, the `Installed` section says `No harnesses are installed.` and the `Available` section shows the reviewed choices.
Use `--json` to receive the same installed and available inventory as a schema-versioned object.

### `nemoclaw harness install [id]`

Install one reviewed harness package bundled with the current NemoClaw build.
Pass the exact ID shown by `nemoclaw harness list` for scripts and other non-interactive sessions.

```bash
nemoclaw harness install openclaw
nemoclaw harness install example \
  --from /path/to/nemoclaw-example \
  --yes-i-trust-local-package
```

In an interactive terminal, omit `[id]` to choose from reviewed packages that are not already installed.
The command copies and verifies the selected package, then records its exact identity in the current user's NemoClaw state.
Installing the same exact package again is safe and keeps that identity active.

The normal install path accepts only reviewed artifacts carried by the installed NemoClaw build.
It does not accept a URL, remote package source, or catalog-refresh option.

The experimental `--from` path accepts one local built package directory and requires its canonical package ID.
Review the package before you pass `--yes-i-trust-local-package` because package adapters run as trusted code during later operations.
The install command validates and stores the artifact, but it does not run package lifecycle scripts or package tests.

### `nemoclaw harness activate <id>`

Activate an installed package revision for new sandboxes.
This experimental development command moves the package's active pointer to an immutable receipt that is already in the local package store.

```bash
nemoclaw harness activate openclaw --digest sha256:<content-digest>
nemoclaw harness activate openclaw --digest sha256:<content-digest> --json
```

The command verifies the package ID, digest, receipt, and stored content before it changes the active pointer.
Existing sandboxes remain bound to their recorded package receipt.

### `nemoclaw harness remove <id>`

Deactivate a harness package for new sandboxes without deleting its immutable package history.

```bash
nemoclaw harness remove openclaw --yes
nemoclaw harness remove openclaw --yes --json
```

Omit `--yes` in an interactive terminal to confirm the change at the prompt.
Non-interactive and JSON invocations require `--yes` when the package is active.
Use `nemoclaw harness activate` with a retained digest to activate that revision again.

### `nemoclaw harness validate <artifact-directory>`

Inspect one built agent runtime package as static data.
This experimental development command checks the package tree, metadata envelope, agent manifest, data-only agent definition, content digest, and final filesystem authority.

```bash
nemoclaw harness validate ./dist/nemoclaw-example
nemoclaw harness validate ./dist/nemoclaw-example --json
```

Pass the built artifact directory, not its source repository.
The validator rejects source-only content such as tests, `.git`, and `node_modules`.
It does not install the package, import or run package code, run package tests or lifecycle scripts, or add the package to `nemoclaw harness list`.

Use `--json` for the schema-versioned validation report, including the package identity and content digest, manifest path, runtime kind, entry count, and total bytes.
A successful result establishes static artifact conformance only.
It does not qualify an image build, OpenShell startup, inference request, or agent turn.
...
For Hermes, the backed-up paths come from `packages/nemoclaw-hermes/manifest.yaml`, including `/sandbox/.hermes` state such as memories, sessions, skills, plugins, cron, scripts, logs, plans, workspace, messaging platform state, `runtime/state.db`, and the default kanban board in `kanban.db`.
...
These commands do not appear in the command-level parity check, which compares `$$nemoclaw --help` against the public command headings in this reference; hidden commands are excluded from both.
The table above is the canonical reference for the script-backed family.
The experimental adapter is documented separately because it has no owning script.

`$$nemoclaw internal voice-gateway serve` is registered for the experimental adapter described below.
The current OpenClaw package declares the required semantic-turn command.
Hermes and Deep Agents Code do not declare one.
...
The trusted launcher fixes the runtime deployment, runtime profile, sandbox, and package target.
It resolves the semantic-turn command, timeout, package receipt, gateway, and sandbox lifecycle authority from NemoClaw state.
The runtime cannot select an agent, package session key, native endpoint, or forwarding destination.

The launcher sets the exact feature gate `NEMOCLAW_EXPERIMENTAL_VOICE_GATEWAY=1` for its child.
...
Before starting the adapter, the bounded voice-gateway launcher opens one owner-only regular file without following symbolic links.
It maps only this inherited file descriptor into the child process:

- Descriptor `3` supplies the deployment credential used during session admission.

The descriptor number is fixed and is not a configurable flag.
The command rejects a missing, non-regular, wrong-owner, group-accessible, malformed, or oversized input.
It reads the descriptor once and closes it before accepting traffic.
The deployment credential remains in process memory until the voice-gateway process stops.
The launcher does not place its credential source path or value in arguments or environment variables.
It inherits only credential descriptor `3` across this exec, then closes its parent copy.
To rotate the deployment credential, stop the command and restart it with a newly opened descriptor.
The trusted caller must create, replace, remove, and revoke the credential source.
Closing the descriptor or stopping the gateway does not remove the source file or revoke its value.

The launcher is an internal library boundary for trusted external integrations, not another public CLI command or part of normal NemoClaw managed startup.
Callers use the shipped `runVoiceGatewayLaunch()` action with trusted source paths and runtime fields.
Package-contract coverage launches the real internal command through that production entry point and verifies descriptor cleanup and restart-based credential rotation.
...
The authority and timeout arguments are generated by core and are not caller configuration.
Do not run this child command directly because it requires the launcher's descriptor mapping and captured authority.
...
NEMOCLAW_EXPERIMENTAL_VOICE_GATEWAY=1 $$nemoclaw internal voice-gateway serve \
  ...
  --sandbox-authority <launcher-generated-binding> \
  --agent <package-agent-id> \
  --turn-timeout-ms <receipt-declared-timeout-ms> \
  --listen-port 18800
```
...
For each committed turn, NemoClaw takes the sandbox mutation lock and resolves the published sandbox authority again.
The package must declare a managed `runtime.semantic_turn` command that uses the `semantic-turn-ndjson` protocol.
NemoClaw refuses a change to the sandbox, exact package receipt, gateway name or port, lifecycle generation, or live identity fingerprint.
It sends the request through OpenShell standard input, enforces the package timeout and core stream limits, and cancels the command when the session closes.
The package command can return only `started`, `text`, `completed`, or `failed` events.
An absent declaration, unpublished registration, changed receipt, malformed event, or native command failure fails the turn without loading harness code into core.

The OpenClaw package implements this command with `runtime/semantic-turn.sh` and `runtime/semantic-turn.mts`.
The shell wrapper accepts only the protected gateway environment created by OpenClaw managed startup.
The TypeScript adapter maps the opaque conversation key and package target into OpenClaw's session grammar.
It keeps the OpenClaw credential, WebSocket frames, native traffic limits, response correlation, and run ID inside the package.
...
Sequential sessions for the same runtime conversation reuse the same internal package context.
...
The core gateway process owns voice-session admission, the one-turn state machine, and the opaque conversation key.
The package command owns only its native session mapping and response correlation for that turn.
Core clears the voice-session state when the session closes or expires and when the foreground process stops.
