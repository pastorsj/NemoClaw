...
If status reports an incomplete add transaction, rerun the original `mcp add` command with the same URL, environment-variable name, and any `--deny-tool` options.
...
Confirm that no `mcp add`, `mcp update`, `mcp restart`, `mcp remove`, `rebuild`, or `destroy` command for the sandbox is still running, then retry the original command.
