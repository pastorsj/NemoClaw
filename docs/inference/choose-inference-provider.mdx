...
<Warning>
On N1x, the Deferred managed-vLLM preview is the only admitted inference route.
Hosted providers, compatible endpoints, Model Router, Ollama, existing vLLM or llama.cpp servers, managed llama.cpp, fixed vLLM profiles, and NVIDIA NIM are unavailable.
...
| Local vLLM already running | On hosts other than N1x, NemoClaw detects a server on `localhost:${NEMOCLAW_VLLM_PORT:-8000}`. | [Set Up vLLM](../local-inference/set-up-vllm) |
...
| Local NVIDIA NIM | Unavailable on N1x. On other hosts, a NIM-capable NVIDIA GPU must be present and experimental setup must be enabled. | [Set Up NVIDIA NIM](../local-inference/set-up-nvidia-nim) |
</AgentOnly>
