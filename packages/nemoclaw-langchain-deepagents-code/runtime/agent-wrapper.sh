...
# Used by the package-owned status workflow sourced below.
...
load_agent_status_workflow() {
  local status_module="/usr/local/lib/nemoclaw/dcode-agent-status.sh"
  if [ ! -f "$status_module" ]; then
    status_module="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)/agent-status.sh"
  fi
  if [ ! -f "$status_module" ] || [ -L "$status_module" ] || [ ! -r "$status_module" ]; then
    printf 'dcode: refusing to start — managed status workflow is unavailable.\n' >&2
    exit 2
  fi
  # shellcheck source=agent-status.sh
  source "$status_module"
}

case "${1:-}" in
  status | whoami | identity | --help | -h | help | --version | -v | -V)
    load_agent_status_workflow "$@"
    ;;
esac
