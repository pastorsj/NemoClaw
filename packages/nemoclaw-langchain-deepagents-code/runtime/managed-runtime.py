...
# material. Mirrors runtime/agent-wrapper.sh is_otlp_endpoint_name (#6466).
...
# Keep this managed adapter allow-list in sync with config/generate-config.ts and
# the compat/runtime-patch.py provider guards injected into Deep Agents Code.
...
# tests/runtime/proxy-launcher.test.ts prevent validator drift.
...
# Regression gate: tests/host/secret-parity.test.ts
...
# same corpus in tests/image/credentials.test.ts.
