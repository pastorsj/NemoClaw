...
const BUILTIN_ONLY_PROFILE_STATE = MANAGED_MODEL_ALIASES.map((_alias, index) => index === 0);
...
function createInstalledPluginWheelFixture(version: string): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-profile-wheel-"));
  tempRoots.push(root);
  const wheelRoot = path.join(root, "wheel-root");
  const installRoot = path.join(root, "installed");
  const distribution = `nemoclaw_deepagents_profile-${version}`;
  const distributionMetadata = `${distribution}.dist-info`;
  const packageDirectory = path.join(wheelRoot, "nemoclaw_deepagents_profile");

  fs.mkdirSync(packageDirectory, { recursive: true });
  fs.copyFileSync(pluginSourcePath, path.join(packageDirectory, "__init__.py"));
  writeFixtureFile(
    wheelRoot,
  ...
  writeFixtureFile(
    wheelRoot,
    `${distributionMetadata}/WHEEL`,
    [
      "Wheel-Version: 1.0",
      "Generator: NemoClaw test fixture",
  ...
  writeFixtureFile(
    wheelRoot,
    `${distributionMetadata}/entry_points.txt`,
  ...
  const wheelPath = path.join(root, `${distribution}-py3-none-any.whl`);
  execFileSync(
    pythonBin,
    [
      "-S",
      ...
      [
        ...
        "with zipfile.ZipFile(sys.argv[2], 'w', zipfile.ZIP_DEFLATED) as wheel:",
        "    for entry in sorted(root.rglob('*')):",
        "        if entry.is_file():",
        "            wheel.write(entry, entry.relative_to(root).as_posix())",
      ].join("\n"),
      wheelRoot,
      wheelPath,
    ],
...
if state == "complete":
    ...
    _HARNESS_PROFILES[aliases[1]] = canonical
elif state == "conflict":
    for key in aliases:
...
describe("LangChain Deep Agents Code managed Nemotron profile plugin (#6424)", () => {
  ...
  it("rejects an installed real plugin wheel with an unreviewed version", () => {
    ...
    const installedPluginRoot = createInstalledPluginWheelFixture("0.1.1");
  ...
  it.each([
    ...
    expect(result.probe.aliases).toEqual(BUILTIN_ONLY_PROFILE_STATE);
    expectOfficialSourcesUnchanged(fixture);
  });
  ...
  it("rolls back the first alias when the second registration fails", () => {
    ...
    expect(result.probe.registryKeys).toEqual(
      [CANONICAL_MODEL_SPEC, MANAGED_MODEL_ALIASES[0], "openai:gpt-4.1-mini"].sort(),
    );
    expectOfficialSourcesUnchanged(fixture);
  });
});
