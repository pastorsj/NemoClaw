...
const checkPath = path.join(packageRoot, "checks", "profile-gate.sh");
const profileGateDockerfiles = [
  {
    scriptPath: "packages/nemoclaw-langchain-deepagents-code/Dockerfile.base",
    sourcePath: path.join(packageRoot, "Dockerfile.base"),
  },
  {
    scriptPath: "test/Dockerfile.dcode-profile-missing-dependencies",
    sourcePath: path.join(packageRoot, "tests", "fixtures", "Dockerfile.profile-missing"),
  },
  {
    scriptPath: "packages/nemoclaw-langchain-deepagents-code/Dockerfile",
    sourcePath: path.join(packageRoot, "Dockerfile"),
  },
] as const;
const reviewedDockerfiles = profileGateDockerfiles.map(({ scriptPath }) => scriptPath);
...
function runGateWithFakeDocker(
  ...
  const fixtureCheckPath = path.join(
    fixtureRoot,
    "packages",
    "nemoclaw-langchain-deepagents-code",
    "checks",
    path.basename(checkPath),
  );
  ...
  for (const dockerfile of profileGateDockerfiles) {
    const fixturePath = path.join(fixtureRoot, dockerfile.scriptPath);
    fs.mkdirSync(path.dirname(fixturePath), { recursive: true });
    fs.copyFileSync(dockerfile.sourcePath, fixturePath);
  }
