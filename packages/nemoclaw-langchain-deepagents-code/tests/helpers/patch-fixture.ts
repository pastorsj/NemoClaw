...
import { addDarwinFcntlSealConstants } from "./darwin-seal.ts";

export const agentDir = path.resolve(import.meta.dirname, "../..");
...
`,
  ...
  const appFixture = fs
    .readFileSync(path.join(agentDir, "tests", "fixtures", "disclosure", "app.py"), "utf8")
    .replace(
      "class DeepAgentsApp:\n",
...
`,
  ...
  writeFixtureFile(
    ...
    fs.readFileSync(path.join(agentDir, "tests", "fixtures", "disclosure", "server.py"), "utf8"),
  );
  writeFixtureFile(
    packageDir,
    "_server_config.py",
...
`,
  ...
  writeFixtureFile(
    packageDir,
    "mcp_tools.py",
    fs.readFileSync(path.join(agentDir, "tests", "fixtures", "disclosure", "mcp_tools.py"), "utf8"),
  );
  writeFixtureFile(
    packageDir,
    "hooks/__init__.py",
