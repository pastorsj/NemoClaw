...
import { addDarwinFcntlSealConstants } from "../helpers/darwin-seal.ts";
import { runStartScriptProxyProbe, TRUSTED_FETCH_PROXY_ENV_NAME } from "../helpers/headless.ts";
import {
  cleanupPackageFixtures,
  createPackageFixture,
  patchFixture,
} from "../helpers/patch-fixture.ts";
...
describe("LangChain Deep Agents Code managed fetch proxy", () => {
  ...
  it("rejects consistently forged proxy env that differs from root-owned files", () => {
    ...
    try {
      ...
      fs.writeFileSync(
        runtimeFile,
        addDarwinFcntlSealConstants(readAgentFile("runtime/managed-runtime.py")),
        "utf8",
      );
