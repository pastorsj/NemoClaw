...
RUNTIME_SOURCE_DIR = Path(__file__).parent.parent / "runtime"
MANAGED_RUNTIME_SOURCE_PATH = RUNTIME_SOURCE_DIR / "managed-runtime.py"
MIDDLEWARE_SOURCE_PATH = RUNTIME_SOURCE_DIR / "tool-disclosure.py"
OBSERVABILITY_SOURCE_PATH = RUNTIME_SOURCE_DIR / "observability.py"
...
# Ultra compatibility workarounds" in compat/dependencies.md.
...
def create_cli_agent(model, assistant_id, *args, **kwargs):
    ...
    # input_config); checks/observability.py locks that merge path.
...
def main() -> None:
    ...
    module_destination_path, module_source = _load_managed_module(
        root, MIDDLEWARE_MODULE, MIDDLEWARE_SOURCE_PATH, "middleware"
    )
