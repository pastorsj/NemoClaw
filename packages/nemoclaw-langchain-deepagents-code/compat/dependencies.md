...
- Lockfile: `packages/nemoclaw-langchain-deepagents-code/runtime/requirements.lock`
- Lockfile SHA-256: `c0ebf0697dad505dfe9d074a88e2c06c83b599f146c96af77dd3e7b183154da9`
- Audit command: `uv tool run --python 3.13 pip-audit -r packages/nemoclaw-langchain-deepagents-code/runtime/requirements.lock --progress-spinner off --disable-pip`
...
> `tests/config/profile-plugin.test.ts` as soon as the
...
The focused plugin suite extracts the identifiers from every production
...
template argument at its own boundary. `config/generate-config.ts` writes the per-model
...
`packages/nemoclaw-langchain-deepagents-code/checks/profile-gate.sh` with real Docker before live tests;
