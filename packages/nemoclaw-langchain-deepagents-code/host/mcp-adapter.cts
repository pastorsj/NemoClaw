...
const DEEPAGENTS_MCP_CAPABILITY_COMMAND = "/usr/local/bin/deepagents-code --nemoclaw-mcp-capability";
...
function buildMcpSnapshotRestorePlan(request) {
    if (request.entries.length > DEEPAGENTS_MCP_MAX_SERVERS) {
        throw new Error("Deep Agents managed MCP supports at most " +
    ...
    return {
        ...
        verificationFailureMessage: "Deep Agents Code managed MCP projection verification failed after snapshot repair",
    };
}
...
function buildRegisterCommand(entry, replaceExisting = false, managedEntries = [entry], teardownRollback = false) {
    ...
    if (teardownRollback)
        return buildRollbackRegisterCommand(entry, expectedServers);
    const payload = {
        server: entry.server,
        expected: managedServerConfig(entry),
        expectedServers,
...
function buildMcpRegistrationCommand(request) {
    return buildRegisterCommand(request.entry, request.replaceExisting, request.managedEntries, request.teardownRollback);
}
