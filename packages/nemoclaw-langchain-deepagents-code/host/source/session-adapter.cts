...
function buildSessionMutationPlan(
  ...
  return {
    kind: "unsupported",
    reason: `LangChain Deep Agents Code does not expose session ${request.operation} through NemoClaw yet.`,
  };
}
...
function buildSessionExportPlan(
  ...
  return {
    kind: "unsupported",
    reason: "LangChain Deep Agents Code does not expose session export through NemoClaw yet.",
  };
}
