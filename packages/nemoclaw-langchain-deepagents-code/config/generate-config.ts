...
const { normalizeManagedDcodeEndpointUrl, resolveManagedDcodeIdentity } =
  require("../host/managed-identity.cts") as ManagedIdentityRuntime;
...
type FabricModelProvider = "nvidia" | "openai-compatible";

type ManagedFabricConfig = {
  ...
  environment: {
    ...
    metadata?: {
      nemoclaw: {
        invocation_unavailable_reason: string;
      };
    };
  };
  models: {
...
const FABRIC_ARTIFACTS_PATH = "/sandbox/.deepagents/fabric-artifacts";
...
function openAiModelRequestParamLines(
  ...
  // compat/dependencies.md under "Managed Ultra compatibility workarounds."
...
function selectFabricModelProvider(upstreamProvider: string): FabricModelProvider {
  const normalizedProvider = upstreamProvider.toLowerCase();
  return normalizedProvider === "nvidia" || normalizedProvider.startsWith("nvidia-")
    ? "nvidia"
    : "openai-compatible";
}

function fabricInvocationUnavailableReason(
  settings: Settings,
  identity: ManagedDcodeIdentity,
): string | null {
  const unavailableOptions = [
    ...(NEMOTRON_ULTRA_MODEL_IDS.has(identity.model)
      ? ["the managed Nemotron Ultra force_nonempty_content option"]
      : []),
    ...(settings.reasoningEffort ? [`NEMOCLAW_REASONING_EFFORT=${settings.reasoningEffort}`] : []),
  ];
  if (unavailableOptions.length === 0) return null;
  return (
    `The released Fabric 0.2 Deep Agents adapter cannot apply ${unavailableOptions.join(
      " and ",
    )}. ` + "Use native dcode -n for this model configuration."
  );
}
...
function buildFabricConfig(
  ...
  const invocationUnavailableReason = fabricInvocationUnavailableReason(settings, identity);
  return {
    schema_version: "fabric.agent/v1alpha1",
    metadata: {
    ...
    environment: {
      ...
      ownership: "caller_owned",
      control_location: "in_env_control",
      ...(invocationUnavailableReason
        ? {
            metadata: {
              nemoclaw: {
                invocation_unavailable_reason: invocationUnavailableReason,
              },
            },
          }
        : {}),
    ...
    models: {
      default: {
        provider: selectFabricModelProvider(settings.upstreamProvider),
...
function buildConfig(settings: Settings): ManagedDeepAgentsConfig {
  const identity = resolveManagedDcodeIdentity(
    settings.upstreamProvider,
    settings.model,
    settings.upstreamEndpointUrl,
  );
  const { provider, model, defaultModel } = identity;
  const text = [
  ...
  const fabricText = `${JSON.stringify(buildFabricConfig(settings, identity), null, 2)}\n`;
  return { text, fabricText, provider, model, defaultModel };
}
...
function main(): void {
  ...
  writeFileSync(fabricConfigPath, config.fabricText);
  chmodSync(fabricConfigPath, 0o600);

  console.log(
    `[config] Wrote ${configPath} and ${fabricConfigPath} (model=${config.defaultModel}, base_url=${settings.baseUrl})`,
  );
}
