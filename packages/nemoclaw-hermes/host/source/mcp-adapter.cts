...
function buildIntentPayload(entries: any, managedServerNames: any): any {
  ...
  const absent = [...new Set<string>(managedServerNames)]
    .filter((name) => !presentNames.has(name))
    .sort();
  return { present, absent };
}

function buildRegisterCommand(entry: any, replaceExisting: any = false): any {
  const payload = {
    server: entry.server,
    url: entry.url,
...
function buildRemoveCommand(entry: any, force: any = false): any {
  const payload = {
    server: entry.server,
    url: entry.url,
    headers: entry.headers,
...
function buildProbeCommand(): any {
  return [HERMES_MCP_TRANSACTION_HELPER, "probe"];
}

function buildInspectCommand(payload: any): any {
...
function describeMcpTeardownCapability(
  request: Parameters<HarnessMcpAdapterModule["describeMcpTeardownCapability"]>[0],
): ReturnType<HarnessMcpAdapterModule["describeMcpTeardownCapability"]> {
  return describeMcpMutationCapability(request);
}

function describeMcpRuntimeIntentVerification(
  request: Parameters<HarnessMcpAdapterModule["describeMcpRuntimeIntentVerification"]>[0],
): ReturnType<HarnessMcpAdapterModule["describeMcpRuntimeIntentVerification"]> {
