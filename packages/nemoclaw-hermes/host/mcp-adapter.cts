...
function packageArgvCommand(argv) {
...
function buildIntentPayload(entries, managedServerNames) {
    ...
    const present = Object.fromEntries(sortedEntries.map((entry) => [entry.server, managedServerConfig(entry)]));
    const presentNames = new Set(Object.keys(present));
    const absent = [...new Set(managedServerNames)]
...
function buildMcpRuntimePlan(request) {
    const runner = "import subprocess, sys; raise SystemExit(subprocess.run(sys.argv[1:], check=False).returncode)";
    return {
