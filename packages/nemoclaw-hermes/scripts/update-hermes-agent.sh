...
#
# Usage:
#   packages/nemoclaw-hermes/scripts/update-hermes-agent.sh                  # pin latest GitHub release
#   packages/nemoclaw-hermes/scripts/update-hermes-agent.sh --tag v2026.6.19 # pin a specific calver tag
#   packages/nemoclaw-hermes/scripts/update-hermes-agent.sh --check          # exit 0 if up-to-date, 1 if stale
#   packages/nemoclaw-hermes/scripts/update-hermes-agent.sh --build          # also build the base image (no cache)
#   packages/nemoclaw-hermes/scripts/update-hermes-agent.sh --rebuild        # --build + sandbox rebuild + verify
#   packages/nemoclaw-hermes/scripts/update-hermes-agent.sh --update-installed-copies
...
if [[ "$CHECK_ONLY" == 1 ]]; then
  ...
  [[ "$status" == 0 ]] || echo "To update, run: packages/nemoclaw-hermes/scripts/update-hermes-agent.sh"
  exit "$status"
fi
...
if [[ "$DO_REBUILD" == 1 ]]; then
  ...
  echo "  packages/nemoclaw-hermes/scripts/update-hermes-agent.sh --tag ${TAG} --build     # build base image without cache"
  echo "  packages/nemoclaw-hermes/scripts/update-hermes-agent.sh --tag ${TAG} --rebuild   # build + sandbox rebuild + verify"
  echo "  packages/nemoclaw-hermes/scripts/update-hermes-agent.sh --tag ${TAG} --update-installed-copies"
fi
