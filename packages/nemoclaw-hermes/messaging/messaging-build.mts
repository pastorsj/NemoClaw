...
function removeReviewedNpmArchive(archive) {
  rmSync(archive.rootDirectory, { recursive: !0, force: !0 });
}
...
function readReviewedLock(lockfilePath) {
  ...
  let packages = lockRecord.packages;
...
function parseExactPackageSpec(packageSpec) {
  ...
  let separator = packageSpec.lastIndexOf("@");
...
function packageNameFromLockLocation(location) {
  let marker = "node_modules/",
    nestedMarkerIndex = location.lastIndexOf(`/${marker}`),
    markerIndex =
      nestedMarkerIndex >= 0 ? nestedMarkerIndex + 1 : location.startsWith(marker) ? 0 : -1,
    packageName = markerIndex >= 0 ? location.slice(markerIndex + marker.length) : "";
  if (!packageName)
...
function lockDependencies(record, location) {
  ...
  let peerDependencies = record.peerDependencies,
    peerDependenciesMeta = record.peerDependenciesMeta;
  if (
  ...
  if (peerDependencies !== void 0) {
    ...
    for (let name of Object.keys(peerDependencies)) {
      if (!EXACT_NPM_PACKAGE_SPEC.test(`${name}@0.0.0`))
        throw new Error(
          `reviewed npm lock has an invalid dependency name: ${location || "root package"}: ${name}`,
        );
      let peerMeta = peerDependenciesMeta?.[name];
      if (
        peerMeta !== void 0 &&
      ...
      let optional = peerMeta?.optional === !0;
      dependencies.has(name) || dependencies.set(name, optional);
    }
  }
  return [...dependencies].map(([name, optional]) => ({ name, optional }));
}
function assertNotProductionDev(productionLocations, location, record) {
  if (productionLocations?.has(location) && record.dev === !0)
...
function resolveLockDependencyLocation(packages, requesterLocation, dependencyName) {
  ...
  for (;;) {
    let candidate = ancestorLocation
    ...
    if (!ancestorLocation) return;
    let parentMarker = ancestorLocation.lastIndexOf("/node_modules/");
...
function productionLockLocations(packages) {
  ...
  let reachable = /* @__PURE__ */ new Set(),
    pending = [{ location: "", record: root }];
  for (; pending.length > 0;) {
    let current = pending.pop();
    if (!current) break;
    for (let dependency of lockDependencies(current.record, current.location)) {
      let location = resolveLockDependencyLocation(packages, current.location, dependency.name);
      if (!location) {
...
function verifyReviewedLockDigest(lockfilePath, expectedLockSha256, label) {
  ...
  let actualLockSha256 = createHash("sha256").update(readFileSync(lockfilePath)).digest("hex");
...
function readReviewedLockPackages(
  ...
  let reviewed = [],
    ...
    reviewedRegistryIdentities = /* @__PURE__ */ new Map(),
    allowedNestedShrinkwrapIdentities = new Set(allowedNestedShrinkwrapPackages),
    reviewedPackagesWithoutIntegrityBySpec = new Map(
      reviewedPackagesWithoutIntegrity.map((reviewed2) => [reviewed2.packageSpec, reviewed2]),
    );
  for (let reviewedPackage of reviewedRegistryPackages) {
  ...
  let productionLocations = omitDev ? productionLockLocations(packages) : void 0;
  for (let [location, value] of Object.entries(packages)) {
    if (location === "") continue;
    if (typeof value != "object" || value === null || Array.isArray(value))
      throw new Error(`reviewed npm lock has an invalid package record: ${location}`);
    let record = value;
    if (
      (assertNotProductionDev(productionLocations, location, record), omitDev && record.dev === !0)
    )
      continue;
    let locationName = packageNameFromLockLocation(location),
      packageName = typeof record.name == "string" ? record.name : locationName,
    ...
    let expectedIntegrity = typeof record.integrity == "string" ? record.integrity : "",
      tarballUrl = typeof record.resolved == "string" ? record.resolved : "",
      reviewedPackageWithoutIntegrity = reviewedPackagesWithoutIntegrityBySpec.get(packageSpec);
    ...
    let reviewedRegistryIdentity = reviewedRegistryIdentities.get(packageSpec);
    ...
    let prior = identities.get(packageSpec);
    if (prior) {
    ...
    (identities.set(packageSpec, request),
      reviewedPackageWithoutIntegrity || reviewed.push(request));
  }
...
function verifyReviewedNpmLock(request, npmRunner = runNpm) {
  ...
  let registryOrigin = normalizeRegistryOrigin(request.registryOrigin),
    packages = readReviewedLock(request.lockfilePath),
    { name, version } = parseExactPackageSpec(request.packageSpec),
    root = packages[""];
  if (!root || typeof root != "object" || Array.isArray(root))
    throw new Error("reviewed npm lock is missing its root package record");
  let rootDependencies = root.dependencies,
  ...
  let topLevel = packages[`node_modules/${name}`];
  ...
  let reviewed = readReviewedLockPackages(packages, request.lockfilePath, registryOrigin);
  return (
    verifyReviewedNpmMetadata(request, npmRunner), reviewed.map(({ packageSpec }) => packageSpec)
  );
}
function verifyInstalledNpmLock(request) {
  verifyReviewedLockDigest(request.lockfilePath, request.expectedLockSha256, request.label);
  let packages = readReviewedLock(request.lockfilePath),
    installRoot = resolve(request.installRoot),
    nodeModulesRoot = resolve(installRoot, "node_modules"),
    verified = [],
    productionLocations = request.omitDev ? productionLockLocations(packages) : void 0;
  for (let [location, record] of Object.entries(packages)) {
    if (location === "") continue;
    if (typeof record != "object" || record === null || Array.isArray(record))
    ...
    if (
      ...
      request.omitDev && record.dev === !0)
    )
      continue;
    let locationName = packageNameFromLockLocation(location),
      expectedName = typeof record.name == "string" ? record.name : locationName,
      expectedVersion = typeof record.version == "string" ? record.version : "",
      packageSpec = `${expectedName}@${expectedVersion}`;
    if (!EXACT_NPM_PACKAGE_SPEC.test(packageSpec))
      throw new Error(`${request.label} has an invalid locked package identity: ${packageSpec}`);
    let packageDirectory = resolve(installRoot, location);
    ...
    let manifestPath = join(packageDirectory, "package.json"),
      manifest,
      manifestDescriptor;
    try {
      manifestDescriptor = openSync(manifestPath, "r");
      let openedManifestEntry = fstatSync(manifestDescriptor),
        pathManifestEntry = lstatSync(manifestPath);
      if (
      ...
      manifestDescriptor !== void 0 && closeSync(manifestDescriptor);
    }
...
function verifyReviewedNpmCache(request, npmRunner = runNpm) {
  ...
  let cacheDirectory = resolve(request.cacheDirectory);
  if (!existsSync(cacheDirectory))
    throw new Error(`reviewed npm cache does not exist: ${cacheDirectory}`);
  let cache = lstatSync(cacheDirectory);
  ...
  let registryOrigin = normalizeRegistryOrigin(request.registryOrigin),
    packages = readReviewedLockPackages(
  ...
  for (let reviewed of packages) {
    ...
    (removeReviewedNpmArchive(archive), verified.push(reviewed.packageSpec));
  }
  return verified;
}
function parseCliOptions(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--verify-installed-lock") {
      verifyInstalledLock = !0;
      continue;
    }
    if (!arg?.startsWith("--")) throw new Error(`Unknown argument: ${arg ?? ""}`);
    let value = argv[index + 1];
    if (!value || value.startsWith("--")) throw new Error(`${arg} requires a value`);
    (values.set(arg, value), (index += 1));
  }
  let required = (name) => {
