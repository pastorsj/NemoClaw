...
#   6. With --update-installed-copies, asks the NemoClaw harness inventory whether
#      the installed Hermes package matches this build, then scans legacy source
#      locations for saved Dockerfiles and manifests. Content-addressed packages
#      are refreshed only through `nemoclaw harness install hermes`.
#
...
#
# Usage:
#   packages/nemoclaw-hermes/checks/update-agent.sh                  # pin latest GitHub release
#   packages/nemoclaw-hermes/checks/update-agent.sh --tag v2026.6.19 # pin a specific calver tag
#   packages/nemoclaw-hermes/checks/update-agent.sh --check          # exit 0 if up-to-date, 1 if stale
#   packages/nemoclaw-hermes/checks/update-agent.sh --build          # also build the base image (no cache)
#   packages/nemoclaw-hermes/checks/update-agent.sh --rebuild        # --build + sandbox rebuild + verify
#   packages/nemoclaw-hermes/checks/update-agent.sh --update-installed-copies
...
#   NEMOCLAW_CLI           — optional path to the NemoClaw CLI used for the
#                            installed harness inventory (default: nemoclaw)
...
safe_installed_dockerfile() {
  ...
  if [[ -e "$receipt" || -L "$receipt" ]]; then
    echo "SKIP legacy receipt-managed package ${file}: refresh it with nemoclaw harness install" >&2
    return 1
  fi
...
discover_installed_dockerfiles() {
  ...
  for root in "${roots[@]}"; do
    ...
    find "$root" -maxdepth 6 \
      \( -path '*/.nemoclaw/harnesses' -o -name node_modules -o -name .git \) -prune \
...
verify_installed_hermes_package() {
  local nemoclaw_cli inventory status
  nemoclaw_cli="${NEMOCLAW_CLI:-}"
  if [[ -z "$nemoclaw_cli" ]]; then
    nemoclaw_cli="$(command -v nemoclaw || true)"
  fi
  if [[ -z "$nemoclaw_cli" ]]; then
    echo "Installed harness inventory skipped because the NemoClaw CLI is unavailable."
    return 0
  fi
  if [[ ! -x "$nemoclaw_cli" ]]; then
    echo "INVALID: configured NemoClaw CLI is not executable." >&2
    return 1
  fi
  if ! inventory="$("$nemoclaw_cli" harness list --json)"; then
    echo "INVALID: NemoClaw could not read the installed harness inventory." >&2
    return 1
  fi
  if ! status="$(node -e '
let input = "";
...
process.stdin.on("end", () => {
  try {
    ...
    const installed = Array.isArray(inventory.installed)
      ? inventory.installed.find((entry) => entry?.id === "hermes")
      : undefined;
    const available = Array.isArray(inventory.available)
      ? inventory.available.find((entry) => entry?.identity?.id === "hermes")
      : undefined;
    if (!installed) return process.stdout.write("absent");
    if (installed.health === "damaged") return process.stdout.write("damaged");
    if (installed.health !== "healthy" || !available) throw new Error("invalid inventory");
    if (available.installationState === "active") return process.stdout.write("active");
    if (available.installationState === "different") return process.stdout.write("different");
    throw new Error("invalid Hermes installation state");
  } catch {
    process.exitCode = 1;
  }
});
' <<<"$inventory")"; then
    echo "INVALID: NemoClaw returned an invalid harness inventory." >&2
    return 1
  fi
  case "$status" in
    absent) return 0 ;;
    active)
      echo "OK: installed Hermes harness package matches this NemoClaw build."
      ;;
    different)
      echo "STALE: installed Hermes harness package differs from this NemoClaw build." >&2
      echo "Refresh it with: nemoclaw harness install hermes" >&2
      return 1
      ;;
    damaged)
      echo "INVALID: installed Hermes harness package failed integrity validation." >&2
      echo "Refresh it with: nemoclaw harness install hermes" >&2
      return 1
      ;;
    *)
      echo "INVALID: NemoClaw returned an unknown Hermes harness state." >&2
      return 1
      ;;
  esac
}
...
if [[ "$CHECK_ONLY" == 1 ]]; then
  ...
  if [[ "$UPDATE_INSTALLED_COPIES" == 1 ]]; then
    verify_installed_hermes_package || status=1
  ...
  [[ "$status" == 0 ]] || echo "To update, run: packages/nemoclaw-hermes/checks/update-agent.sh"
  exit "$status"
fi
...
# ---------------------------------------------------------------------------
# Verify the generic NemoClaw harness inventory, then bring legacy installed-source
...
if [[ "$UPDATE_INSTALLED_COPIES" == 1 || "$DO_BUILD" == 1 ]]; then
  verify_installed_hermes_package || exit 1
fi
if [[ "$UPDATE_INSTALLED_COPIES" == 1 ]]; then
  while IFS= read -r installed_df; do
    [[ -n "$installed_df" ]] || continue
...
if [[ "$DO_REBUILD" == 1 ]]; then
  ...
  echo "  packages/nemoclaw-hermes/checks/update-agent.sh --tag ${TAG} --build     # build base image without cache"
  echo "  packages/nemoclaw-hermes/checks/update-agent.sh --tag ${TAG} --rebuild   # build + sandbox rebuild + verify"
  echo "  packages/nemoclaw-hermes/checks/update-agent.sh --tag ${TAG} --update-installed-copies"
fi
