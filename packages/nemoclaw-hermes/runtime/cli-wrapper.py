...
#     `packages/nemoclaw-hermes/start.sh` uses for `_HERMES_BOUNDARY_VALIDATOR`.
#
...
#     `packages/nemoclaw-hermes/config/managed-policy.ts:buildHermesManagedPolicy`, the literal
...
#   - Regression test: `tests/runtime/gateway-wrapper.test.ts` —
...
#
# `runtime/cli-adapter.json` owns the exact translated command forms, their
...
# entrypoint (`packages/nemoclaw-hermes/start.sh:validate_hermes_runtime_env_secret_boundary`)
...
# same priority is mirrored by `packages/nemoclaw-hermes/start.sh:resolve_trusted_python3`
