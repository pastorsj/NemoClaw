...
import type { HermesBuildSettings } from "../../config/build-env.ts";
import { buildHermesManagedPolicy } from "../../config/managed-policy.ts";

const HERMES_PACKAGE_ROOT = path.resolve(import.meta.dirname, "../..");
const patcher = path.join(HERMES_PACKAGE_ROOT, "compat", "profile-policy.py");
const imageBuildProbes = path.join(HERMES_PACKAGE_ROOT, "checks", "image-probes.py");
...
`;
  ...
  // source-shape-contract: security -- Hash and probe bindings fail closed when reviewed profile source shapes drift
  it.each([
    ...
    (expectedSourceHash) => {
      ...
      expect(dockerfile).toContain(
        "COPY packages/nemoclaw-hermes/compat/profile-policy.py " +
          "/opt/nemoclaw-hermes-config/patch-profile-policy-defaults.py",
      );
