...
import { MIN_HERMES_CONTEXT_WINDOW, readHermesBuildSettings } from "../../config/build-env.ts";
...
import { discoverModelSpecificSetups } from "../../config/model-setup.ts";
import {
...
const CONFIG_MODULE_DIR = path.join(import.meta.dirname, "../..", "config");
...
function copyConfigGeneratorFixture(fixtureRoot: string): string {
  ...
  fs.cpSync(
    path.join(import.meta.dirname, "../..", "host"),
...
function findRawSecretEnvEntries(envFile: string): string[] {
  ...
  // packages/nemoclaw-hermes/runtime/env-boundary.py. API_SERVER_KEY is the bearer
...
describe("packages/nemoclaw-hermes/config/generate-config.ts", () => {
  ...
  it("writes Discord settings without persisting its injected credential", async () => {
  ...
  it("leaves minimal persisted credential bindings to the OpenShell process environment", () => {
    const plan = {
      schemaVersion: 1,
    ...
    const { envFile } = generateConfigWithMessaging(
      buildHermesTestEnv({
        NEMOCLAW_MESSAGING_CHANNELS_B64: encodeJson([]),
  ...
  it("enables Slack and Telegram without persisting their injected credentials", async () => {
    const { config, envFile } = await runConfigScriptWithMessaging({
  ...
  it("bridges captured WeChat metadata to Hermes' WEIXIN_* env contract", async () => {
    ...
    // The runtime plan maps OpenShell's WECHAT_BOT_TOKEN process value to
    // Hermes' WEIXIN_TOKEN name. Persisting either key in .env would shadow
    // the revision-scoped value injected at startup.
  ...
  it("discovers the bundled registry from the script path when cwd differs", () => {
    const sourceRegistryDir = path.join(import.meta.dirname, "../..", "model-specific-setup");
