...
const READER_PATH = path.join(import.meta.dirname, "../..", "runtime", "managed_policy.py");
const PROFILE_PATCHER_PATH = path.join(import.meta.dirname, "../..", "compat", "profile-policy.py");
...
describe("Hermes managed policy", () => {
  it("serializes one versioned policy without persisting messaging credentials (#8008)", () => {
    ...
    expect(policy.env_lines.some((line) => line.startsWith("DISCORD_BOT_TOKEN="))).toBe(false);
