...
const PATCHER = path.join(HERMES_PACKAGE_ROOT, "compat", "gateway-metadata.py");
const MCP_TRANSACTION = path.join(HERMES_PACKAGE_ROOT, "runtime", "mcp-transaction.py");
...
describe("Hermes writable gateway runtime metadata", () => {
  ...
  // source-shape-contract: compatibility -- The packaged MCP transaction must keep reading the relocated gateway identity record
