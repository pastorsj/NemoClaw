...
const PLUGIN_PATH = path.join(import.meta.dirname, "../..", "plugin", "__init__.py");
...
module._tool_broker.open = fake_open
...
module._tool_broker._load_hermes_dotenv()
...
plugin = importlib.util.module_from_spec(spec)
spec.loader.exec_module(plugin)
broker = plugin._tool_broker
broker._load_hermes_config = lambda: {"security": {"allow_private_urls": False}}
...
patched = broker._install_broker_url_safety_patch()
print(json.dumps({
    "accepted": [
        broker._broker_safe_url("https://example.com/path"),
        broker._broker_safe_url("https://8.8.8.8/dns-query"),
    ],
    "blocked": [broker._broker_safe_url(url) for url in blocked_urls],
    "patched": patched,
    "same_predicate": [
        url_safety.is_safe_url is broker._broker_safe_url,
        web_tools.is_safe_url is broker._broker_safe_url,
        browser_tool._is_safe_url is broker._broker_safe_url,
    ],
