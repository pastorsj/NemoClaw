...
import { extractShellFunction, readHermesStartupSource } from "../helpers/shell-harness";

const HERMES_PACKAGE_ROOT = path.resolve(import.meta.dirname, "../..");
const patcher = path.join(HERMES_PACKAGE_ROOT, "compat", "discord-permissions.py");
const dockerfile = fs.readFileSync(path.join(HERMES_PACKAGE_ROOT, "Dockerfile"), "utf8");
const imageBuildProbes = fs.readFileSync(
  path.join(HERMES_PACKAGE_ROOT, "checks", "image-probes.py"),
  "utf8",
);
const baseDockerfile = fs.readFileSync(path.join(HERMES_PACKAGE_ROOT, "Dockerfile.base"), "utf8");
...
function runCrossUidParentRepair(name: "gateway" | "runtime", kind: "symlink" | "file") {
  ...
  return spawnSync("bash", [script], {
    encoding: "utf8",
    env: { HERMES_DIR: hermesHome, PATH: process.env.PATH ?? "" },
    timeout: 5000,
  });
}
...
describe("Hermes cross-UID ledger permissions", () => {
  ...
  // source-shape-contract: security -- Digest and image instructions bind the reviewed recovery patch to shipped bytes
  it("hash-binds and executes the patcher in the final image", () => {
    ...
    expect(dockerfile).toContain(
      "COPY packages/nemoclaw-hermes/compat/discord-permissions.py " +
  ...
  // source-shape-contract: security -- Cross-identity probe instructions preserve the reviewed cron ledger ownership boundary
  ...
  // source-shape-contract: security -- Shipped probe instructions bind backup, replacement, and reopen recovery behavior
