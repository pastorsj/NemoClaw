...
const HERMES_DOCKERFILE = path.join(HERMES_PACKAGE_ROOT, "Dockerfile");
const HERMES_BUILD_MCP_DIGEST = path.join(HERMES_PACKAGE_ROOT, "runtime", "mcp-digest.py");
const HERMES_RUNTIME_CONFIG_GUARD = path.join(HERMES_PACKAGE_ROOT, "runtime", "config-guard.py");
...
describe("Hermes doctor and config hash boundary", () => {
  ...
  it("removes stale OpenClaw preloads and locks trusted gateway recovery helpers", () => {
    ...
    try {
      ...
      fs.writeFileSync(runtimeStateMutationTransportBrokerPath, "# transport broker fixture\n");
