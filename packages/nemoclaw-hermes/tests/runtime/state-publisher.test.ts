...
import { readHermesStartupSource } from "../helpers/shell-harness";

const HERMES_PACKAGE_ROOT = path.resolve(import.meta.dirname, "../..");
const PUBLISHER = path.join(HERMES_PACKAGE_ROOT, "runtime", "state", "publisher.py");
const CAPABILITY = path.join(HERMES_PACKAGE_ROOT, "runtime", "state", "publisher.json");
const STATE_PLAN = path.join(HERMES_PACKAGE_ROOT, "runtime", "state", "plan.json");
const START = path.join(HERMES_PACKAGE_ROOT, "start.sh");
const STARTUP_GATE = path.join(HERMES_PACKAGE_ROOT, "runtime", "state", "startup-gate.py");
...
describe("Hermes runtime state mutation publisher", () => {
  ...
  it("fails closed with stable host recovery guidance for invalid gate state (#10155)", () => {
    ...
    try {
      ...
      expect(result.stderr).toContain("Run 'nemoclaw <sandbox-name> shields status' on the host");
