...
const a2aNeutralPatch = fs.readFileSync(path.join(HERMES_PACKAGE_ROOT, "a2a-neutral.patch"));
...
describe("Hermes image build probes", () => {
  ...
  it("binds the runtime environment validator to its source digest", () => {
    const imageDockerfile = fs.readFileSync(path.join(HERMES_PACKAGE_ROOT, "Dockerfile"), "utf8");
    const runtimeEnvValidator = fs.readFileSync(
      path.join(HERMES_PACKAGE_ROOT, "runtime", "env-boundary.py"),
    );
  ...
  it("binds the Hermes wrapper to its source digest", () => {
    ...
    const hermesWrapper = fs.readFileSync(
      path.join(HERMES_PACKAGE_ROOT, "runtime", "cli-wrapper.py"),
    );
  ...
  it.each(commands)(
    ...
    (command) => {
      ...
      expect(dockerfile).toContain(
        `COPY packages/nemoclaw-hermes/checks/image-probes.py ${imageProbePath}`,
      );
