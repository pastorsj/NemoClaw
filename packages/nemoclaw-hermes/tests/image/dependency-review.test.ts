...
import { dockerfileInstructions } from "../../../../test/helpers/dockerfile-run-commands";

const HERMES_PACKAGE_ROOT = path.resolve(import.meta.dirname, "../..");
const dockerfileBase = fs.readFileSync(path.join(HERMES_PACKAGE_ROOT, "Dockerfile.base"), "utf8");
const dockerfile = fs.readFileSync(path.join(HERMES_PACKAGE_ROOT, "Dockerfile"), "utf8");
const config = fs.readFileSync(
  path.join(HERMES_PACKAGE_ROOT, "config", "managed-policy.ts"),
  "utf8",
);
const manifest = fs.readFileSync(path.join(HERMES_PACKAGE_ROOT, "manifest.yaml"), "utf8");
const cliAdapter = JSON.parse(
  fs.readFileSync(path.join(HERMES_PACKAGE_ROOT, "runtime", "cli-adapter.json"), "utf8"),
);
const review = fs.readFileSync(path.join(HERMES_PACKAGE_ROOT, "compat", "dependencies.md"), "utf8");
...
const runtimeBoundariesPatch = fs.readFileSync(
  path.join(HERMES_PACKAGE_ROOT, "compat", "runtime-boundaries.patch"),
  "utf8",
);
const hindsightProbeRequirementsPath = path.join(
  HERMES_PACKAGE_ROOT,
  "checks",
  "hindsight-requirements.txt",
);
...
describe("Hermes 0.19.0 dependency review", () => {
  // source-shape-contract: security -- Exact release identities and image digests bind the reviewed dependency input set
  ...
  // source-shape-contract: security -- Exact authorization and state fields bind the reviewed Hermes security posture
  ...
  // source-shape-contract: compatibility -- Adapter metadata must stay aligned with the exact Hermes CLI translation surface
  ...
  // source-shape-contract: security -- Exact remediation commands and hashes bind the reviewed Python dependency graph
  it("ships the reviewed Python dependency remediations and records residual debt", () => {
    ...
    expect(dockerfileBase).toContain(
      "COPY packages/nemoclaw-hermes/compat/runtime-boundaries.patch /tmp/hermes-runtime-boundaries.patch",
    );
    expect(dockerfile).toContain(
      "COPY packages/nemoclaw-hermes/compat/runtime-boundaries.patch /scripts/hermes-runtime-boundaries.patch",
    );
  ...
  // source-shape-contract: security -- Exact image instructions keep gateway dependency installation offline and isolated from sandbox-controlled package paths
