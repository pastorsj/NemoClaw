...
Shields was retired from NemoClaw after this review was written. Every Shields reference below is
retained as historical evidence of the ownership, mode, and failure behavior evaluated for the
original Hermes 0.19 integration; none describes a current command or supported runtime posture.
The current image keeps Hermes configuration and user state mutable, while retaining the ledger
relocation, online backup and restore, credential isolation, root-separated lazy-package and plugin
...
The `BASE_IMAGE` argument in `packages/nemoclaw-hermes/Dockerfile` pins the patched multi-platform Open Container Initiative (OCI) index `sha256:d30f58b0374c7d281df07c126cc257baaf63eb051264a575b96099ec037d5e5d`.
...
| `packages/nemoclaw-hermes/whatsapp-proxy.patch` | `c154223` |
