...
work_dir="$(mktemp -d "${TMPDIR:-/tmp}/nemoclaw-deepseek-fabric-tests.XXXXXX")"
test_mode="${1:-adapter}"
...
cp -R "${package_root}/fabric" "${work_dir}/nemoclaw-deepseek-fabric"
find "${work_dir}/nemoclaw-deepseek-fabric" \
...
uv_args=(
  ...
  --with "${work_dir}/nemoclaw-deepseek-fabric"
)
test_args=(python tests/fabric/test_adapter.py DeepSeekRuntimeTests ProcessWorkerTests)
...
if [ "${test_mode}" = composed ]; then
  ...
  [ -f "${runner_source}/pyproject.toml" ] || {
    printf 'composed Fabric tests require the generic nemoclaw-fabric source\n' >&2
    exit 66
  }
