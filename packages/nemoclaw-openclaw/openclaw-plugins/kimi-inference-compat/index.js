...
function isReasoningContentBlock(value) {
  ...
  return (
    REASONING_CONTENT_TYPES.has(type) || type.includes("reasoning") || type.includes("thinking")
  );
}
...
function buildSplitToolCallId(originalId, index, command) {
  const rawId =
    typeof originalId === "string" && originalId.trim() ? originalId.trim() : "kimi_exec";
...
function rewriteSafeCombinedExecToolCallsInContent(content) {
  ...
  for (const block of content) {
    ...
    if (split) {
      expanded.push(
        ...dedupeSafeExecToolCalls(buildSplitToolCalls(split.toolCall, split.commands)),
      );
      changed = true;
    } else {
      expanded.push(block);
...
function targetSplitCommandIndex(event, split) {
  ...
  const content =
    event && event.partial && Array.isArray(event.partial.content) ? event.partial.content : [];
