...
import { writeReviewedNpmFixture } from "../../../../test/helpers/reviewed-npm-fixture";
...
it.runIf(process.platform === "linux")(
  ...
  () => {
    ...
    const temporaryDirectory = fs.mkdtempSync(
      path.join(os.tmpdir(), "nemoclaw-messaging-cache-safety-"),
    );
    const unsafeCacheEntry = path.join(temporaryDirectory, "unsafe-cache-entry");
    const installCacheTemplate = path.join(temporaryDirectory, "install-cache.XXXXXX");
...
it.each([
  ...
  const temporaryDirectory = fs.mkdtempSync(
    path.join(os.tmpdir(), "nemoclaw-messaging-phase-selection-"),
  );
  const trustedCache = path.join(temporaryDirectory, "trusted-cache");
  const installCacheTemplate = path.join(temporaryDirectory, "install-cache.XXXXXX");
