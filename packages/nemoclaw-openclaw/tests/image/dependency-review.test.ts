...
check_not_contains "$optional_plugin_block" 'pack_reviewed_npm_tarball' "optional plugin inline pack helper"
	...
	check_not_contains "$(cat "$messaging_build_applier")" 'packages/nemoclaw-openclaw' "messaging applier package import"
	grep -Fq 'runPackageOwnedNodeArchiveRemediation({' "$messaging_build_applier"
	grep -Fq 'NEMOCLAW_NODE_PACKAGE_REMEDIATION_HELPER=/packages/nemoclaw-openclaw/compat/npm-remediation.mts' "$openclaw_dockerfile"
