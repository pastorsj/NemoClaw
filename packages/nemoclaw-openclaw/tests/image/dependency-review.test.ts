...
import {
  ...
  type WorkflowJob,
  type WorkflowStep,
} from "../../../../test/helpers/e2e-workflow-contract";
...
describe("OpenClaw 2026.6.10 dependency review contract", () => {
  it("pins the active diagnostics Jaeger remediation to the shipped install path", () => {
    ...
    expect(review).toContain("packages/nemoclaw-openclaw/tests/compat/jaeger-runtime.test.ts");
  ...
  it("records the version-scoped transient remote MCP startup recovery patch (#7958)", () => {
    ...
    expect(review).toContain("packages/nemoclaw-openclaw/compat/mcp-reliability.mts");
    expect(review).toContain(
    ...
    expect(review).toContain("packages/nemoclaw-openclaw/tests/compat/mcp-reliability.test.ts");
    expect(review).toContain("packages/nemoclaw-openclaw/tests/helpers/mcp-proof.ts");
  ...
  it("records the version-scoped managed outbound transport diagnostics patch (#7957)", () => {
    ...
    expect(review).toContain("packages/nemoclaw-openclaw/compat/transport-diagnostics.mts");
    ...
    expect(review).toContain(
      "packages/nemoclaw-openclaw/tests/compat/transport-diagnostics.test.ts",
    );
  ...
  it("records the bounded MCP tool discovery timeout patch (#7957)", () => {
    ...
    expect(review).toContain("packages/nemoclaw-openclaw/compat/mcp-timeout.mts");
    ...
    expect(review).toContain("packages/nemoclaw-openclaw/tests/compat/mcp-timeout.test.ts");
  ...
  it("records the active mcporter advisory remediations", () => {
    ...
    expect(review).toContain("`fast-uri@3.1.6`");
    expect(review).toContain("GHSA-5jgf-p345-68v8");
    expect(review).toContain("GHSA-f65p-4m7j-42xc");
    expect(review).toContain("GHSA-fph4-wmhf-6fwf");
    expect(review).toContain("GHSA-jqff-g426-hqxp");
    expect(review).toContain("`qs@6.15.3`");
    expect(review).toContain("GHSA-x5fp-wj9c-mxmx");
    expect(review).toContain("GHSA-4mjr-xmp4-gh2g");
  ...
  it("keeps advisor disposition evidence in the dependency review note", () => {
    ...
    expect(review).toContain("packages/nemoclaw-openclaw/tests/compat/dist-harness.test.ts");
    expect(review).toContain("NEMOCLAW_REAL_OPENCLAW_DIST_HARNESS=1");
    ...
    expect(review).toContain("packages/nemoclaw-openclaw/tests/compat/diagnostics-4434.test.ts");
    expect(review).toContain("packages/nemoclaw-openclaw/compat/inference-diagnostics.mts");
    expect(review).toContain("packages/nemoclaw-openclaw/compat/device-approval.mts");
...
remediation_helper=packages/nemoclaw-openclaw/compat/npm-remediation.mts
...
check_contains "$optional_plugin_block" '/packages/nemoclaw-openclaw/compat/npm-remediation.mts' "optional plugin remediation helper"
...
check_not_contains "$optional_plugin_block" 'pack_reviewed_npm_tarball' "optional plugin inline pack helper"
	...
	grep -Fq 'from "../../../../../packages/nemoclaw-openclaw/compat/npm-remediation.mts"' "$messaging_build_applier"
	grep -Fq 'remediateReviewedOpenClawPluginArchive({' "$messaging_build_applier"
	grep -Fq 'NEMOCLAW_OPENCLAW_NPM_REMEDIATION_HELPER=/packages/nemoclaw-openclaw/compat/npm-remediation.mts' "$openclaw_dockerfile"
	...
	grep -Fq 'OPENSHELL_SANDBOX !== "1"' "$issue_4434_patch"
		...
		grep -Fq 'COPY packages/nemoclaw-openclaw/compat/inference-diagnostics.mts /usr/local/lib/nemoclaw/patch-openclaw-issue-4434-diagnostics.mts' "$openclaw_dockerfile"
		...
		grep -Fq 'COPY packages/nemoclaw-openclaw/compat/tool-catalog.mts /usr/local/lib/nemoclaw/patch-openclaw-tool-catalog.mts' "$openclaw_dockerfile"
		...
		grep -Fq 'COPY packages/nemoclaw-openclaw/compat/device-approval.mts /usr/local/lib/nemoclaw/patch-openclaw-device-self-approval.mts' "$openclaw_dockerfile"
	...
	grep -Fq 'COPY packages/nemoclaw-openclaw/compat/state-permissions.mts /usr/local/lib/nemoclaw/patch-openclaw-shared-state-permissions.mts' "$openclaw_dockerfile"
	...
	grep -Fq 'COPY packages/nemoclaw-openclaw/compat/mcp-reliability.mts /usr/local/lib/nemoclaw/patch-openclaw-mcp-reliability.mts' "$openclaw_dockerfile"
	...
	grep -Fq 'COPY packages/nemoclaw-openclaw/compat/mcp-timeout.mts /usr/local/lib/nemoclaw/patch-openclaw-mcp-tools-list-timeout.mts' "$openclaw_dockerfile"
