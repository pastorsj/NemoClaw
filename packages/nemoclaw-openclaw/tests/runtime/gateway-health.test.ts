...
// gateway is covered in gateway-watchdog.test.ts. Marker tests are kept in
// gateway-marker.test.ts so each package test follows one runtime workflow.
...
import {
  ...
  gatewayLaunchBlock,
  gatewayMarkerFunction,
  GATEWAY_SUPERVISOR,
  pidIdentityFunctions,
  readFileIfPresent,
  rootGatewayLifecycleFunctions,
  safeTmpHelpers,
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          "#!/usr/bin/env bash",
          `_NEMOCLAW_PROC_ROOT=${JSON.stringify(procRoot)}`,
          extractShellFunction(readOpenClawStartupSource(), "gateway_pid_is_openclaw_gateway"),
...
describe("respawn loop pidfile refresh (#4710)", () => {
  function respawnLoop(src: string, kind: "non-root" | "root"): string {
    ...
    expect(start, `Expected ${kind} respawn loop in packages/nemoclaw-openclaw/start.sh`).not.toBe(
      -1,
    );
    ...
    expect(
      ...
      `Expected ${kind} respawn loop terminator in packages/nemoclaw-openclaw/start.sh`,
    ).not.toBe(-1);
