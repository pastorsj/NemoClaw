...
const OPENCLAW_STARTUP_ENV = path.join(PACKAGE_ROOT, "runtime", "startup-env.sh");
const MERGE_END = "# Git TLS CA bundle fix (NemoClaw#2270).";
const setup = resolveCaSetup("openclaw-corporate-ca-tls");

afterAll(() => cleanupCaSetup(setup));

describe.skipIf(!setup.ok)("OpenClaw corporate proxy CA TLS verification (#6210)", () => {
