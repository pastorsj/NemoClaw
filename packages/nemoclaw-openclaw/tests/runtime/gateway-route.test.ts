...
describe("gateway dial-back base policy", () => {
  function loadYaml(packagePath: string): Record<string, unknown> {
    return YAML.parse(
      fs.readFileSync(path.join(import.meta.dirname, "..", "..", packagePath), "utf-8"),
    ) as Record<string, unknown>;
  }

  function dialbackEndpoints(): Array<Record<string, unknown>> {
    const policy = loadYaml("policy-additions.yaml");
