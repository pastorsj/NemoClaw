...
// Gateway-health coverage remains separate so each suite follows one runtime
// workflow and neither test becomes a monolith.
...
function runWatchdog(opts: {
  ...
  // Wait for this many completed probes instead of relying on wall time.
  settleProbes?: number;
  ...
  const probeCountFile = path.join(tmpDir, "probe-count.txt");
  ...
  const wrapper = [
    ...
    ...(opts.curlUnavailable
      ...
      : [
          ...
          `_CURL_COUNT=${JSON.stringify(probeCountFile)}`,
          "curl() {",
          "  local next rest code status count",
          '  count="$(cat "$_CURL_COUNT" 2>/dev/null || printf 0)"',
          '  printf "%s\\n" "$((count + 1))" >"$_CURL_COUNT"',
    ...
    ...(opts.expectKill
      ...
      : opts.settleProbes
        ? [
            `for _ in $(command seq 1 300); do [ "$(cat "$_CURL_COUNT")" -ge ${opts.settleProbes} ] && break; command sleep 0.02; done`,
            `[ "$(cat "$_CURL_COUNT")" -ge ${opts.settleProbes} ] || { printf "watchdog completed too few probes\\n" >&2; exit 89; }`,
          ]
...
describe("gateway serving watchdog (#4710, #7377)", () => {
  ...
  it("does not kill a PID whose cmdline no longer looks like the gateway", () => {
    const { result, fakeAlive, tmpDir } = runWatchdog({
      ...
      expectKill: false,
      // The sixth probe starts only after the watchdog has classified the
      // fifth probe and emitted the PID mismatch diagnostic.
      settleProbes: 6,
    });
    try {
      expect(result.status, `script failed: ${result.stderr}`).toBe(0);
