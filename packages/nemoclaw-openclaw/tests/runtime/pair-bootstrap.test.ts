...
import { readOpenClawAutoPairSource, readOpenClawStartupSource } from "../helpers/startup";
...
describe("nemoclaw-start initial CLI auto-pair bootstrap (#6113)", () => {
  ...
  it("does not let the background watcher or its CLI children inherit container stdin", () => {
    ...
    expect(startupSource).toMatch(/python3 -u "\$_OPENCLAW_AUTO_PAIR_SCRIPT" <\/dev\/null/);
  });
