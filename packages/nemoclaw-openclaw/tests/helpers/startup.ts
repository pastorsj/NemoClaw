...
export function readOpenClawStartupBlock(startMarker: string, endMarker: string): string {
  const source = readOpenClawStartupSource();
  const start = source.indexOf(startMarker);
  const end = source.indexOf(endMarker, start);
  if (start < 0 || end < start) {
    throw new Error(`OpenClaw startup does not contain the block before ${endMarker}.`);
  }
