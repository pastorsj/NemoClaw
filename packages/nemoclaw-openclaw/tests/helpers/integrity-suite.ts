...
import { testTimeoutOptions } from "./timeouts";
...
function runInstallBlock(
  ...
  const script = [
    ...
    command
      ...
      .replaceAll("/packages/nemoclaw-openclaw/compat/npm-remediation.mts", remediationHelper)
      .replaceAll("/scripts/lib/reviewed-npm-audit.mts", auditHelper)
      .replaceAll("/scripts/npm-audit-exceptions.json", auditExceptionFile)
      // Docker stages use fixed /tmp names safely because each build has its
      // own filesystem. Concurrent process fixtures need per-test paths.
      .replaceAll(
...
function runOptionalOpenClawPluginBlock(
  ...
  const script = [
    ...
    command
      ...
      .replaceAll("/scripts/lib/reviewed-npm-archive.mts", REVIEWED_NPM_ARCHIVE_HELPER)
      .replaceAll("/packages/nemoclaw-openclaw/compat/npm-remediation.mts", remediationFixture),
  ].join("\n");
  const scriptPath = path.join(tmp, "run.sh");
  fs.writeFileSync(scriptPath, script, { mode: 0o700 });
  const result = spawnSync("bash", [scriptPath], { encoding: "utf-8", timeout: 10000 });
  ...
  describe("OpenClaw npm integrity pins", () => {
    if (group === "contract") {
      it("keeps the advisory review note aligned with the committed OpenClaw pin", () => {
        ...
        expect(reviewNote).toContain(
          "packages/nemoclaw-openclaw/tests/compat/dist-harness.test.ts",
        );
        ...
        expect(reviewNote).toContain("packages/nemoclaw-openclaw/compat/inference-diagnostics.mts");
        expect(reviewNote).toContain("packages/nemoclaw-openclaw/compat/device-approval.mts");
        expect(reviewNote).toContain("packages/nemoclaw-openclaw/compat/state-permissions.mts");
    ...
    if (group === "contract") {
      ...
      it(
        "production build arg guard rejects current reviewed pin overrides",
    ...
    if (group === "plugin-install") {
      ...
      it("keeps optional OpenClaw plugins checksum-sourced, SRI-verified, and offline", () => {
        ...
        const remediation = fs.readFileSync(
          path.join(REPO_ROOT, "packages", "nemoclaw-openclaw", "compat", "npm-remediation.mts"),
          "utf-8",
        );
