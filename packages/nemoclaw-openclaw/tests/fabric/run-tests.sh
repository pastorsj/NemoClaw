...
work_dir="$(mktemp -d "${TMPDIR:-/tmp}/nemoclaw-openclaw-fabric-tests.XXXXXX")"
test_mode="${1:-adapter}"
...
# Build the adapter from a disposable copy. Setuptools can create build
# metadata beside its input, and package tests must leave the checkout clean.
cp -R "${package_root}/fabric" "${work_dir}/nemoclaw-openclaw-fabric"
find "${work_dir}/nemoclaw-openclaw-fabric" \
...
uv_args=(
  ...
  --with "${work_dir}/nemoclaw-openclaw-fabric"
)
test_args=(python tests/fabric/test_adapter.py OpenClawRuntimeTests ProcessWrapperTests)
