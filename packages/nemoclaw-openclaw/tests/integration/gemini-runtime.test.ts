...
import { main } from "../../config/generate-config.mts";
...
const PLUGIN_SOURCE_PATH = path.join(PACKAGE_ROOT, "openclaw-plugins", "gemini-inference-compat");
const CONFIG_INSTALL_PATH = "/opt/nemoclaw-test-openclaw.json";
...
const config = JSON.parse(fs.readFileSync("/opt/nemoclaw-test-openclaw.json", "utf8"));
...
let stagedPluginBundlePath: string;
...
function runPinnedRuntime(entrypoint: string, args: readonly string[]) {
  const createResult = dockerSpawnSync(
    [
      "create",
      "--network",
      "none",
  ...
  try {
    const pluginCopyResult = dockerSpawnSync(
      ["cp", stagedPluginBundlePath, `${containerId}:/usr/local/share/nemoclaw/`],
      { encoding: "utf8", timeout: 30_000 },
    );
    if (pluginCopyResult.status !== 0) return pluginCopyResult;

    const configCopyResult = dockerSpawnSync(
      ["cp", generatedConfigPath, `${containerId}:${CONFIG_INSTALL_PATH}`],
      { encoding: "utf8", timeout: 30_000 },
    );
    if (configCopyResult.status !== 0) return configCopyResult;

    return dockerSpawnSync(["start", "--attach", containerId], {
      encoding: "utf8",
      timeout: 60_000,
    });
  } finally {
    dockerSpawnSync(["rm", "--force", containerId], { stdio: "ignore", timeout: 15_000 });
  }
}
...
suite("OpenClaw Gemini managed-route runtime compatibility", () => {
  beforeAll(
    () => {
      // Docker Desktop resolves bind mounts on the host, where macOS exposes
      // `/var` through its canonical `/private/var` path. Mount the resolved
      // path so the same fixture works with native Linux and Docker Desktop.
      contextDir = fs.realpathSync(
        fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-gemini-runtime-")),
      );
      ensureOpenClawGeminiRuntimeImage(OPENCLAW_RUNTIME_IMAGE);
      stagedPluginBundlePath = path.join(contextDir, "openclaw-plugins");
      stagedPluginPath = path.join(stagedPluginBundlePath, "gemini-inference-compat");
