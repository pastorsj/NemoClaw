...
Update it and `packages/nemoclaw-openclaw/runtime/mcporter/package*.json` together whenever `MCPORTER_VERSION`, its integrity value, a manifest override, or the locked graph changes in `Dockerfile.base` or `Dockerfile`.
...
- Locked graph: `packages/nemoclaw-openclaw/runtime/mcporter/package-lock.json` (npm lockfile version 3).
- Lock regeneration command: `npm --prefix packages/nemoclaw-openclaw/runtime/mcporter install --package-lock-only --ignore-scripts --omit=dev`
- Advisory command: `npm --prefix packages/nemoclaw-openclaw/runtime/mcporter ci --ignore-scripts --omit=dev && node --experimental-strip-types scripts/lib/reviewed-npm-audit.mts --directory packages/nemoclaw-openclaw/runtime/mcporter --exceptions ci/npm-audit-exceptions.json --graph mcporter-runtime --threshold high && npm --prefix packages/nemoclaw-openclaw/runtime/mcporter audit signatures`
...
- Advisory command: `npm ci --ignore-scripts --omit=dev --legacy-peer-deps --prefix packages/nemoclaw-openclaw/runtime/wechat && npm audit --omit=dev --audit-level=low --json --prefix packages/nemoclaw-openclaw/runtime/wechat && npm audit signatures --prefix packages/nemoclaw-openclaw/runtime/wechat`.
