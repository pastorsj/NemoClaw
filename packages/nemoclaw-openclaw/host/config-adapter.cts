...
function applyReasoning(modelEntry, request) {
    const canCarry = request.route.upstreamProvider === "compatible-endpoint" &&
        request.route.api === "openai-completions";
    if (!request.reasoning.explicit && canCarry)
        return;
...
function withUpstreamProviderHeader(providerConfig, upstreamProvider) {
    ...
    for (const key of Object.keys(headers)) {
        if (key.toLowerCase() === UPSTREAM_PROVIDER_HEADER.toLowerCase())
            delete headers[key];
    }
...
function applyInferenceRoute(config, request) {
    ...
    if (request.contextWindow !== null)
        previousModel.contextWindow = request.contextWindow;
