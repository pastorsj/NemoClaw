...
function mcporterHeadersMatchExpected(actual, expected) {
    ...
    for (const [name, value] of Object.entries(expected)) {
        ...
        if (!new RegExp(`^${canonicalPrefix.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&")}v[0-9]{1,20}_${escapedEnvName}$`, "u").test(actualValue)) {
            return false;
        }
    }
    ...
    return (extraName.toLowerCase() === "accept" &&
...
function buildMcpRegistrationCommand(request) {
    return buildRegisterCommand(request.entry, request.replaceExisting, openClawMcporterRoot(request.configDirectory || DEFAULT_OPENCLAW_CONFIG_DIR));
}
...
function buildMcpRemovalCommand(request) {
    return buildRemoveCommand(request.entry, request.force, openClawMcporterRoot(request.configDirectory || DEFAULT_OPENCLAW_CONFIG_DIR));
}
...
function buildMcpInspectionCommand(request) {
    return packageShellCommand(buildInspectCommand(request.entry, request.failOnMismatch, openClawMcporterRoot(request.configDirectory || DEFAULT_OPENCLAW_CONFIG_DIR)));
}
...
function buildMcpRuntimePlan(request) {
    const runner = 'const { spawnSync } = require("node:child_process"); const result = spawnSync(process.argv[1], process.argv.slice(2), { stdio: "inherit" }); process.exit(result.status ?? 1);';
    return {
