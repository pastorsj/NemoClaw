...
function updatePrimaryModel(config: Record<string, unknown>, primaryModelRef: string): void {
...
function positiveReplyBudget(value: unknown): number | undefined {
  return typeof value === "number" && Number.isSafeInteger(value) && value > 0 ? value : undefined;
}

function readPrimaryReplyBudget(config: Record<string, unknown>): number | undefined {
...
function withUpstreamProviderHeader(
...
function readPreviousInferenceApi(config: Record<string, unknown>) {
  const agents = isObject(config.agents) ? config.agents : null;
  const defaults = agents && isObject(agents.defaults) ? agents.defaults : null;
...
function validationCommand(target: HarnessConfigTarget): HarnessExitZeroCommand {
...
function mutableConfigRepair(request: HarnessMutableConfigRequest): HarnessExitZeroCommand | null {
