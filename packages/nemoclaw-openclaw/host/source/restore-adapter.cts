...
function mergeJsonObjects(base: any, overlay: any): any {
...
function mergeOpenClawPluginLoad(
  backupLoad: any,
  currentLoad: any,
  previousImagePluginLoadPaths: any,
): any {
  ...
  const currentPaths = Array.isArray(current.paths)
    ? current.paths.filter((entry: any) => typeof entry === "string")
    : [];
  const backupPaths = Array.isArray(backup.paths)
    ? backup.paths.filter(
        (entry: any) => typeof entry === "string" && !previousImagePluginLoadPaths?.has(entry),
      )
    : [];
...
function mergeOpenClawPluginIdList(
  ...
  currentValue: any,
  previousImagePluginIds: any,
  currentOppositeIds: any = new Set(),
): any {
  ...
  return [
    ...new Set([
      ...(current ?? []),
      ...(backup ?? []).filter(
        (id: any) => !previousImagePluginIds?.has(id) && !currentOppositeIds.has(id),
      ),
    ]),
  ];
}
...
function managedExtensionOwnership(installs: any): any {
  ...
  for (const install of installs) {
    ...
    ids.add(install.id);
    for (const loadPath of install.configPaths) loadPaths.add(loadPath);
  }
  return { ids, loadPaths };
}

function removedImagePluginIds(previous: any, fresh: any): any {
  if (!previous.ids) return undefined;
  return new Set([...previous.ids].filter((id) => !fresh.ids?.has(id)));
}

function mergeOpenClawTools(backupTools: any, currentTools: any): any {
...
function modelEntryId(entry: any): any {
  return isPlainObject(entry) && typeof entry.id === "string" ? entry.id : null;
}

function restoreRuntimeOwnedFields(merged: any, current: any, ownedFields: any): any {
...
function mergeOpenClawModelEntry(backupModel: any, currentModel: any): any {
...
function mergeOpenClawModelArray(backupModels: any, currentModels: any): any {
...
function mergeOpenClawProviderEntry(backupProvider: any, currentProvider: any): any {
...
function mergeOpenClawProviderMap(backupProviders: any, currentProviders: any): any {
...
function mergeOpenClawModels(backupModels: any, currentModels: any): any {
...
function ensureMergedObject(record: any, key: any): any {
...
function readAgentPrimaryModelRef(config: any): any {
...
function updateMainAgentListModel(agents: any, primaryModelRef: any): any {
...
function reconcileAgentPrimaryModel(merged: any, currentConfig: any): any {
...
function configRestoreOwnership(managedChannelNames: any): any {
...
function mergeOpenClawRestoredConfig(
  ...
  if (
    (options.previousManagedExtensions === undefined) !==
...
function mergeConfigState(
  request: Parameters<HarnessConfigRestoreModule["mergeConfigState"]>[0],
): ReturnType<HarnessConfigRestoreModule["mergeConfigState"]> {
  ...
  try {
    ...
    const options =
      previousManagedExtensions === null || freshManagedExtensions === null
        ? {}
        : {
...
export = exportedAdapter satisfies HarnessConfigRestoreModule;
