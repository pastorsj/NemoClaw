...
import type {
  ...
  HarnessAgentRosterJsonValue,
} from "@nvidia/nemoclaw-harness-contract";

type MutableJsonObject = Record<string, HarnessAgentRosterJsonValue>;
...
function isObject(value: unknown): value is HarnessAgentRosterJsonObject {
...
function normalizeManifest(manifest: HarnessAgentRosterJsonObject): {
  readonly agents: readonly MutableJsonObject[];
  readonly defaults?: HarnessAgentRosterJsonValue;
  readonly main?: HarnessAgentRosterJsonValue;
} {
  const unknownKeys = Object.keys(manifest).filter((key) => !ALLOWED_MANIFEST_KEYS.has(key));
  if (unknownKeys.length > 0) {
    throw new Error(
      `agents manifest contains unsupported top-level field "${unknownKeys.sort()[0]}". Allowed: agents, defaults, main`,
    );
  }
  const rawAgents = manifest.agents ?? [];
  if (!Array.isArray(rawAgents)) throw new Error("agents manifest 'agents' must be a list");

  const seen = new Set<string>();
  const agents = rawAgents.map((rawEntry, index) => {
    const label = `agents[${String(index)}]`;
    if (!isObject(rawEntry)) throw new Error(`${label} must be a YAML mapping (object)`);
    const entry: MutableJsonObject = { ...rawEntry };
    const id = entry.id;
    if (typeof id !== "string" || !AGENT_ID_PATTERN.test(id)) {
      throw new Error(
        `${label}.id ${JSON.stringify(id)} must match ${String(AGENT_ID_PATTERN)} (1-32 chars, lowercase alphanumeric, dash, underscore; must start with a letter)`,
      );
    }
    if (id === MAIN_AGENT_ID) {
    ...
    for (const key of Object.keys(entry)) {
      if (!ALLOWED_AGENT_KEYS.has(key)) {
        throw new Error(
          `${label} contains unsupported field "${key}". Allowed: ${[...ALLOWED_AGENT_KEYS].sort().join(", ")}.`,
        );
      }
    }
    for (const pathKey of ["workspace", "agentDir"] as const) {
      const expected = expectedAgentPath(pathKey, id);
      if (entry[pathKey] === undefined) entry[pathKey] = expected;
      if (entry[pathKey] !== expected) {
        throw new Error(
          `${label}.${pathKey} must equal "${expected}" for agent id "${id}", got "${String(entry[pathKey])}"`,
        );
      }
    }
    return entry;
...
function parseCurrentAgents(output: string): OpenClawAgentEntry[] {
...
function rebuildOnlyFields(manifest: ReturnType<typeof normalizeManifest>): string[] {
  const fields: string[] = [];
  if (hasContent(manifest.defaults)) fields.push("defaults");
  if (hasContent(manifest.main)) fields.push("main");
  for (const entry of manifest.agents) {
    const id = String(entry.id);
    if (entry.model !== undefined) fields.push(`agents[${id}].model`);
    if (hasContent(entry.subagents)) fields.push(`agents[${id}].subagents`);
    if (hasContent(entry.tools)) fields.push(`agents[${id}].tools`);
  }
  return fields;
}

function applyPlan(
  manifestValue: HarnessAgentRosterJsonObject,
  ...
  try {
    ...
    const notices = additions.flatMap((addition) => {
      ...
      if (!hasContent(declared?.tools)) return [];
      return [
        {
...
const agentRosterAdapter: HarnessAgentRosterAdapterModule = {
