...
function applicationRuntimePlan(environment) {
    ...
    for (const [name, kind] of APPLICATION_RUNTIME_INPUTS) {
        ...
        if (raw === undefined)
            continue;
        if (raw.includes("\0") || /[\r\n]/u.test(raw))
            fail(`${name} must be single-line text`);
        const value = Number(raw.trim());
        const valid = kind === "positive-safe-integer"
            ? Number.isSafeInteger(value) && value > 0
            : Number.isFinite(value) && value > 0;
        if (!valid) {
            fail(`${name} must be ${kind === "positive-safe-integer" ? "a positive safe integer" : "finite positive seconds"}`);
        }
...
function messagingManagedFiles(plan) {
    ...
    const addTarget = (target) => {
        if (typeof target !== "string")
            return;
        if (target === "openclaw.json")
            files.add(target);
        else if (target.startsWith("~/.openclaw/"))
            files.add(target.slice("~/.openclaw/".length));
...
function buildStartupPlan(adapterRequest) {
    ...
    const agentTimeoutSeconds = required(config.agentTimeoutSeconds, "configuration.agentTimeoutSeconds");
...
function dashboardHostname(value) {
...
function extraAgents(request) {
    ...
    if (raw !== null && encoded !== null)
        fail("extra-agent inputs conflict");
    if (raw === null && encoded === null) {
    ...
    if (Array.isArray(parsed))
        return { agents: parsed, defaults: { subagents: {} }, main: {} };
...
function prepareStartupProfile(request) {
    ...
    if (bind !== null && bind !== "0.0.0.0")
        fail("dashboard bind address is invalid");
    const dashboardHost = dashboardHostname(input.dashboard.url);
    const remote = bind === "0.0.0.0" ||
    ...
    const sampleRate = Number(preparationValue(request, "NEMOCLAW_OPENCLAW_OTEL_SAMPLE_RATE") ?? "1.0");
    ...
    const contextWindow = request.phase === "rebuild" && input.inference.resolvedContextWindow !== null
        ? input.inference.resolvedContextWindow
        : targetChanged
            ? fail(`cannot determine a context window for '${input.inference.selectedProvider ?? candidate.routeProvider}/${input.inference.model}'`)
            : positiveInteger(request, "NEMOCLAW_CONTEXT_WINDOW", previous?.tuning.contextWindow ?? 131_072, 4_194_304);
    const preparedReasoning = request.phase === "rebuild"
        ? input.inference.selectedProvider === "compatible-endpoint" &&
            input.inference.reasoningEnabled === true
        : reasoning === "true";
    const preparedReasoningEffort = request.phase === "rebuild"
        ? input.inference.selectedProvider === "compatible-endpoint" &&
    ...
    const desiredState = {
        configuration: {
            ...
            otel: {
                enabled: otelEnabled !== null && !["0", "false", "no", "off"].includes(otelEnabled.toLowerCase()),
                endpointUrl: preparationValue(request, "NEMOCLAW_OPENCLAW_OTEL_ENDPOINT") ??
                    "http://host.openshell.internal:4318",
                serviceName: preparationValue(request, "NEMOCLAW_OPENCLAW_OTEL_SERVICE_NAME") ?? "openclaw-gateway",
                sampleRate,
            },
