...
"use strict";
// The existing merge helpers preserve OpenClaw's native JSON shape. The public
// merge entrypoint is checked against the exact restore contract below.
...
const MANAGED_WEB_SEARCH_PLUGIN_ENTRIES = new Set(OPENCLAW_CONFIG_RESTORE_OWNERSHIP.managedWebSearchPluginEntries);
...
function mergeOpenClawChannels(backupChannels, currentChannels, managedChannels, previousImagePluginIds) {
    if (!isPlainObject(backupChannels))
        return cloneJson(currentChannels);
    ...
    for (const [key, value] of Object.entries(backupChannels)) {
        ...
        if (managedChannels.has(key) || previousImagePluginIds?.has(key))
            continue;
        const existing = merged[key];
        merged[key] =
...
function mergeOpenClawEntryMap(backupEntries, currentEntries, previousImagePluginIds) {
    if (!isPlainObject(backupEntries) && !isPlainObject(currentEntries))
        return undefined;
    const merged = {};
    if (isPlainObject(backupEntries)) {
        for (const [key, value] of Object.entries(backupEntries)) {
            if (previousImagePluginIds?.has(key))
                continue;
            if (MANAGED_WEB_SEARCH_PLUGIN_ENTRIES.has(key))
                continue;
            merged[key] = cloneJson(value);
        }
    }
...
function mergeOpenClawPluginLoad(backupLoad, currentLoad, previousImagePluginLoadPaths) {
    ...
    const backupPaths = Array.isArray(backup.paths)
        ? backup.paths.filter((entry) => typeof entry === "string" && !previousImagePluginLoadPaths?.has(entry))
        : [];
    const paths = [...new Set([...currentPaths, ...backupPaths])];
    if (Array.isArray(current.paths) || Array.isArray(backup.paths))
        merged.paths = paths;
    else
        delete merged.paths;
...
function mergeOpenClawPluginIdList(backupValue, currentValue, previousImagePluginIds, currentOppositeIds = new Set()) {
    ...
    return [
        ...new Set([
            ...(current ?? []),
            ...(backup ?? []).filter((id) => !previousImagePluginIds?.has(id) && !currentOppositeIds.has(id)),
        ]),
    ];
}
function mergeOpenClawPluginSlots(backupSlots, currentSlots, previousImagePluginIds) {
    if (!isPlainObject(backupSlots) && !isPlainObject(currentSlots))
        return undefined;
    const merged = {};
    if (isPlainObject(backupSlots)) {
        for (const [key, value] of Object.entries(backupSlots)) {
            if (typeof value === "string" && previousImagePluginIds?.has(value))
                continue;
            merged[key] = cloneJson(value);
        }
    }
    if (isPlainObject(currentSlots))
...
function mergeOpenClawTools(backupTools, currentTools) {
    ...
    if ("search" in currentWeb)
        mergedWeb.search = cloneJson(currentWeb.search);
    else
        delete mergedWeb.search;
    if (Object.keys(mergedWeb).length > 0)
        merged.web = mergedWeb;
    else
        delete merged.web;
    for (const field of OPENCLAW_CONFIG_RESTORE_OWNERSHIP.currentGeneratedToolFields) {
        if (field in current)
            merged[field] = cloneJson(current[field]);
        else
            delete merged[field];
    }
    return merged;
...
function mergeOpenClawModelEntry(backupModel, currentModel) {
    ...
    restoreRuntimeOwnedFields(merged, currentModel, OPENCLAW_CONFIG_RESTORE_OWNERSHIP.modelRuntimeOwnedFields);
    return merged;
}
function mergeOpenClawModelArray(backupModels, currentModels) {
    if (!Array.isArray(currentModels))
        return cloneJson(backupModels ?? currentModels);
    const backupById = new Map();
    if (Array.isArray(backupModels)) {
        for (const entry of backupModels) {
            const id = modelEntryId(entry);
            if (id && isPlainObject(entry) && !backupById.has(id))
                backupById.set(id, entry);
        }
    }
...
function mergeOpenClawProviderEntry(backupProvider, currentProvider) {
    ...
    restoreRuntimeOwnedFields(merged, currentProvider, OPENCLAW_CONFIG_RESTORE_OWNERSHIP.providerRuntimeOwnedFields);
...
function mergeOpenClawProviderMap(backupProviders, currentProviders) {
    if (!isPlainObject(backupProviders) && !isPlainObject(currentProviders))
        return undefined;
...
function mergeOpenClawModels(backupModels, currentModels) {
    if (!isPlainObject(backupModels))
        return cloneJson(currentModels);
    if (!isPlainObject(currentModels))
        return cloneJson(backupModels);
...
function mergeOpenClawPlugins(backupPlugins, currentPlugins, previousOwnership, freshOwnership) {
    if (!isPlainObject(backupPlugins) && !isPlainObject(currentPlugins))
        return undefined;
    ...
    const allow = mergeOpenClawPluginIdList(backup.allow, current.allow, backupAllowOwnedIds, new Set(currentDeny ?? []));
    if (allow && allow.length > 0)
        merged.allow = allow;
    else
        delete merged.allow;
    const deny = mergeOpenClawPluginIdList(backup.deny, current.deny, backupDenyOwnedIds, new Set(currentAllow ?? []));
...
function mergeOpenClawRestoredConfig(backedUpConfig, currentConfig, options = {}, managedChannelNames = []) {
    ...
    for (const key of OPENCLAW_CONFIG_RESTORE_OWNERSHIP.runtimeSections) {
        if (key in currentConfig)
            merged[key] = cloneJson(currentConfig[key]);
        else
            delete merged[key];
    }
    merged.channels = mergeOpenClawChannels(backedUpConfig.channels, currentConfig.channels, new Set(managedChannelNames), previousOwnership.ids);
    merged.models = mergeOpenClawModels(backedUpConfig.models, currentConfig.models);
    merged.plugins = mergeOpenClawPlugins(backedUpConfig.plugins, currentConfig.plugins, previousOwnership, freshOwnership);
...
function mergeConfigState(request) {
    ...
    try {
        ...
        return {
            kind: "refused",
            reason: `Selective configuration restore failed: ${error instanceof Error ? error.message : String(error)}`,
        };
    }
}
