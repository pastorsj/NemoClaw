...
//
//   Regression proof: tests/runtime/nemotron-preload.test.ts covers the helper
...
//   the bug.
//
//   Regression proof: tests/runtime/nemotron-preload.test.ts covers the
...
//   test/e2e/docs/4851-ultra-toolless-validation.md — anyone reviewing
...
//
//   Regression proof: tests/runtime/nemotron-preload.test.ts covers the strip/
...
(function () {
  ...
  // `packages/nemoclaw-openclaw/plugin/src/index.ts:WRITE_TOOL_NAMES` so this allowlist stays
  ...
  // `packages/nemoclaw-openclaw/compat/tool-catalog.mts` — when it's present we
  ...
  function isChatCompletionsPost(method, pathOrUrl) {
    return String(method || "GET").toUpperCase() === "POST" && COMPLETIONS_RE.test(pathOrUrl || "");
  }
  ...
  function upstreamProviderFromHeaders(headers) {
    ...
    if (Array.isArray(headers)) {
      for (var i = 0; i < headers.length; i++) {
        var entry = headers[i];
        if (entry && String(entry[0]).toLowerCase() === UPSTREAM_PROVIDER_HEADER) {
  ...
  function isNvidiaBuildUpstream(upstreamProvider) {
    var provider =
      upstreamProvider === undefined ? process.env.NEMOCLAW_UPSTREAM_PROVIDER : upstreamProvider;
  ...
  function wrapFetch() {
    ...
    var wrappedFetch = async function (input, init) {
      ...
      var modified = patchJsonBody(await rawPromise, isManagedBuildFetch(input, upstreamProvider));
      if (!modified) {
        return hasUpstreamProviderMarker
          ? origFetch.call(this, input, nextInit)
          : origFetch.apply(this, arguments);
      ...
      nextInit.headers = headersWithoutContentLength(nextInit.headers || (input && input.headers));
  ...
  function wrapModule(mod) {
    ...
    mod.request = function (options, callback) {
      var isUrlForm =
        typeof options === "string" || (typeof URL !== "undefined" && options instanceof URL);
      var requestOptions =
      ...
      req.end = function (chunk, encoding, cb) {
        ...
        var modified = patchJsonBody(raw, isManagedBuildRequest(options, upstreamProvider));
