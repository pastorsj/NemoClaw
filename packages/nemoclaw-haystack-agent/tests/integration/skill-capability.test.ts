...
describe("Haystack Agent skill capability", () => {
  it("disables skill installation because this package has no native loader", () => {
    const definition = buildAgentDefinition({
      manifest: loadValidatedHarnessManifest(MANIFEST_PATH, "haystack-agent"),
    ...
    expect(definition.skillCapability).toEqual({
      support: "disabled",
      reason: "This package does not implement Haystack Agent skills.",
    });
  });
});
