...
work_dir="$(mktemp -d "${TMPDIR:-/tmp}/nemoclaw-haystack-fabric-tests.XXXXXX")"
test_mode="${1:-adapter}"
...
cp -R "${package_root}/fabric" "${work_dir}/nemoclaw-haystack-fabric"
find "${work_dir}/nemoclaw-haystack-fabric" \
...
uv_args=(
  ...
  --with "${work_dir}/nemoclaw-haystack-fabric"
)
test_args=(python tests/fabric/test_adapter.py HaystackAgentRuntimeTests)
