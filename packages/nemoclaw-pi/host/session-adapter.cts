...
function buildSessionMutationPlan(request) {
  return {
    kind: "unsupported",
    reason: `Pi does not expose session ${request.operation} through NemoClaw yet.`,
  };
}
...
function buildSessionExportPlan(_request) {
  return { kind: "unsupported", reason: "Pi does not expose session export through NemoClaw yet." };
}
