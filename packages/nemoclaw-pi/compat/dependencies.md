...
Update it whenever `packages/nemoclaw-pi/runtime/pi/package-lock.json` changes.
...
- Lockfile: `packages/nemoclaw-pi/runtime/pi/package-lock.json`
- Lockfile SHA-256: `6267ec58e69fc6cd53d3c753f28b0e25c00f4befdcae63e8e4924bee2abf0712`
- Locked packages: 144
- Audit command: `npm --prefix packages/nemoclaw-pi/runtime/pi audit --registry=https://registry.yarnpkg.com --omit=dev`
...
NemoClaw does not check what the Pi runtime does with those values; the deny-by-default network policy in `packages/nemoclaw-pi/policy-additions.yaml` is the enforced control, and it allows only the managed inference route.
