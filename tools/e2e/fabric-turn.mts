...
import type { HostCliClient } from "../../test/e2e/fixtures/clients/host.ts";
import type { SandboxClient } from "../../test/e2e/fixtures/clients/sandbox.ts";
import type { ShellProbeResult } from "../../test/e2e/fixtures/shell-probe.ts";
...
interface PublicFabricTurnBaseOptions {
  ...
  readonly sandbox: Pick<SandboxClient, "exec">;
  readonly sandboxName: string;
...
function requireConfigProbe(
  ...
  return {
    ...
    configMode: String(value.configMode),
    configOwner: String(value.configOwner),
    ...
    descriptorPath: String(value.descriptorPath),
    ...
    stateBytesScanned: stateScanRequired ? Number(value.stateBytesScanned) : null,
    stateCredentialFree: stateScanRequired ? true : null,
    stateEntryCount: stateScanRequired ? Number(value.stateEntryCount) : null,
...
function requireProcessProbe(
  ...
  return {
    ...
    matchingPids: value.matchingPids,
    unreadablePids: value.unreadablePids,
  };
}
