...
function validateDevicePairingSettlement(value: unknown): void {
  if (value === undefined) return;
  const field = "runtime.device_pairing_settlement";
  const declaration = requireRecord(value, field);
  requireKnownFields(
    declaration,
...
export function validateHarnessRuntime(manifest: ManifestRecord): void {
  ...
  if (kind === "terminal" && !interactiveCommand && !headlessCommand) {
    fail("runtime", "must define interactive_command or headless_command for terminal agents");
  }
  if (
  ...
  validateDevicePairingSettlement(runtime.device_pairing_settlement);
  if (manifest.device_pairing === true && runtime.device_pairing_settlement === undefined) {
    fail("runtime.device_pairing_settlement", "is required when device_pairing is enabled");
  }
  if (manifest.device_pairing !== true && runtime.device_pairing_settlement !== undefined) {
    fail("runtime.device_pairing_settlement", "requires device_pairing to be enabled");
  }
  if (
    kind === "terminal" &&
...
export function validateRuntimeSurfaces(manifest: ManifestRecord): void {
  ...
  if (manifest.dashboard_ui !== undefined) {
    ...
    requireKnownFields(
      dashboard,
      new Set(["enable_env", "port", "port_env"]),
      new Set(["enable_env", "port", "port_env"]),
      "dashboard_ui",
    );
    if (
      !Number.isInteger(dashboard.port) ||
      (dashboard.port as number) < 1024 ||
      (dashboard.port as number) > 65_535
    ) {
      fail("dashboard_ui.port", "must be an integer TCP port between 1024 and 65535");
    }
    for (const key of ["enable_env", "port_env"] as const) {
      if (typeof dashboard[key] !== "string" || dashboard[key].trim().length === 0) {
        fail(`dashboard_ui.${key}`, "is required");
      }
    }
  }
}
