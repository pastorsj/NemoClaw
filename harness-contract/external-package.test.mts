...
function createExternalPackage(packageRoot: string, archives: readonly string[]): void {
  ...
  writeFile(
    ...
    [
      ...
      "gateway_command: future-terminal gateway run",
      "health_probe:",
  ...
  writeFile(
    ...
    [
      ...
      'import type { HarnessAgentManifest, HarnessProcessLifecycleDeclaration, HarnessToolGatewayCapability } from "@nvidia/nemoclaw-harness-contract";',
      "",
      ...
      '  gateway_command: "future-terminal gateway run",',
      '  health_probe: { url: "http://127.0.0.1:19090/health", port: 19090, timeout_seconds: 30 },',
      "  forward_ports: [19090],",
      ...
      '  state_lifecycle: { backup_quiescence: { kind: "command", command: ["/usr/local/bin/future-state", "backup-ready"], timeout_seconds: 30 }, snapshot_restore: ["repair-mutable-config"], rebuild: { managed_extensions: { support: "disabled", reason: "No managed extensions." }, scheduled_work: { support: "disabled", reason: "No scheduled work." }, post_restore: { kind: "managed", command: null, reapply_messaging: true, restart_runtime: false, mutable_config: "repair", config_integrity: { kind: "not-required" }, settle_device_pairing: false, notify_gateway_token_change: false } } },',
      ...
      "  policy: { owned_presets: [], automatic_presets: [], baseline_exclusion_impacts: {} },",
      ...
      'const toolGateways: HarnessToolGatewayCapability = { support: "managed", selection_label: "Future managed tools", selection_prompt: "Managed tools", request_environment: ["NEMOCLAW_FUTURE_TOOL_GATEWAYS"], incompatible_auth_message: "Managed tools require browser login.", gateways: [{ id: "future-search", aliases: ["search"], label: "Future search", description: "Search through the future provider", default_selected: true, authentication_methods: ["browser-login"], policy_presets: ["future-search"] }] };',
      'if (toolGateways.support !== "managed" || toolGateways.gateways[0]?.id !== "future-search") throw new Error("Typed tool-gateway declaration was lost");',
  ...
  writeFile(
    ...
    [
      ...
      "const { gateway_command: _gatewayCommand, health_probe: _healthProbe, ...terminalManifest } = manifest;",
      'validateHarnessManifest({ ...terminalManifest, runtime: { kind: "terminal", interactive_command: "future-terminal", headless_command: "node fabric/turn.cjs", prompt_transport: "stdin", process_lifecycle: { support: "unsupported", reason: "This terminal owns no persistent process." } } }, "future-terminal");',
      ...
      'const restored = restore.mergeConfigState({ backupContent: "{\\"user\\":true}", currentContent: "{\\"managed\\":true}", managedChannelNames: [], previousManagedExtensions: null, freshManagedExtensions: null });',
