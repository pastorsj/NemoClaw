...
Package-owned preflight profiles validate mutable configuration before this
shared controller signals a gateway. The fixed trusted profile path is the only
extension point; process discovery, pidfd signaling, and health proof remain
provider-neutral in this module.
...
PROFILE_PATH = "/usr/local/lib/nemoclaw/managed-gateway-profile.py"
...
START_LOG_DIAGNOSTIC_PATTERNS: tuple[re.Pattern[str], ...] = ()
...
def _load_profile():
    global START_LOG_DIAGNOSTIC_PATTERNS, _ACTIVE_PROFILE
    profile_path = _system_path(PROFILE_PATH)
    _validate_trusted_regular(profile_path)
    spec = importlib.util.spec_from_file_location(
        "_nemoclaw_managed_gateway_profile", profile_path
    )
    ...
    for member in ("agent_spec", "gateway_matches", "preflight"):
        if not callable(getattr(module, member, None)):
            raise ControlError("SUPERVISOR_UNAVAILABLE")
    raw_patterns = getattr(module, "DIAGNOSTIC_PATTERNS", ())
    if not isinstance(raw_patterns, tuple) or not all(
        isinstance(pattern, str) for pattern in raw_patterns
    ):
        raise ControlError("SUPERVISOR_UNAVAILABLE")
    try:
        START_LOG_DIAGNOSTIC_PATTERNS = tuple(
            re.compile(pattern) for pattern in raw_patterns
        )
    except re.error as exc:
        raise ControlError("SUPERVISOR_UNAVAILABLE") from exc
    _ACTIVE_PROFILE = module
    return module
...
def _agent_spec(profile, reader: ProcReader, supervisor: ProcessIdentity) -> AgentSpec:
    environment = _parse_environment(
        reader.read_stable_file(supervisor, "environ", MAX_ENV_BYTES)
    )
    try:
        value = profile.agent_spec(environment)
        name = value["name"]
        port = value["port"]
        readiness = value.get("readiness_checks", ())
    ...
    if (
        ...
        or port < 1024
        or port > 65535
        or not isinstance(readiness, tuple)
        or not all(
            isinstance(item, tuple)
            and len(item) == 2
            and isinstance(item[0], int)
            and not isinstance(item[0], bool)
            and 1024 <= item[0] <= 65535
        ...
        raise ControlError("GATEWAY_UNSAFE_CONFIG_PATH")
    return AgentSpec(name=name, port=port, readiness_checks=readiness)
...
def _gateway_matches(
    ...
    if not (
        ...
        return False
    try:
        return bool(_ACTIVE_PROFILE.gateway_matches(identity.cmdline, spec.port))
...
def _preflight(
    ...
    raw_environment = _read_stable_file_with_proof_grace(
        reader, supervisor, "environ", MAX_ENV_BYTES, recovery_deadline
    )
    environment = _parse_environment(raw_environment)
    try:
        error_code = profile.preflight(
            environment, _preflight_timeout(recovery_deadline),
            _system_root(),
        )
    ...
    try:
        ...
        with ProcReader() as reader:
            ...
            with _control_stage("initial-gateway-proof"):
                spec = _agent_spec(profile, reader, supervisor)
                candidates = _gateway_candidates(
                    reader,
                    supervisor,
                    spec,
            ...
            if old_identity is not None:
                ...
                # shutdown once the child has exited: a clean SIGTERM exit can
                # be read by nemoclaw-start as an intentional stop
