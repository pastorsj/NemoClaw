...
git log "$REPORTED_VERSION".."$LATEST" -S"$SYMBOL" \
  --reverse --oneline -- Dockerfile Dockerfile.base src/ bin/ scripts/ agents/ packages/ nemoclaw-blueprint/
...
git show --format=fuller --patch <candidate-sha> -- Dockerfile Dockerfile.base src/ bin/ scripts/ agents/ packages/ nemoclaw-blueprint/ \
  | grep -E '^-[^-]' \
  | grep -nF -- "$SYMBOL"
```
...
SYMBOL=$(<"$EVIDENCE_DIR/symbol.txt")
[ -n "$SYMBOL" ] || { echo "ERROR: reviewed symbol is empty"; exit 1; }
git grep -n -e "$SYMBOL" "$REPORTED_VERSION" -- Dockerfile Dockerfile.base src/ bin/ scripts/ agents/ packages/ nemoclaw-blueprint/
git grep -n -e "$SYMBOL" "$LATEST" -- Dockerfile Dockerfile.base src/ bin/ scripts/ agents/ packages/ nemoclaw-blueprint/
```
...
git grep -n \
  -e "$SYMPTOM_ONE" \
  -e "$SYMPTOM_TWO" \
  "$LATEST" -- Dockerfile Dockerfile.base src/ scripts/ agents/ packages/ nemoclaw-blueprint/
```
...
```bash
git grep -nE "<new-workflow-keyword>" -- test/ agents/ packages/ 2>/dev/null \
