...
while IFS= read -r member; do
  case "$member" in
    dist | dist/* | packages/nemoclaw-openclaw/plugin/dist/shared | packages/nemoclaw-openclaw/plugin/dist/shared/*) ;;
    *)
...
for plugin_parent in \
  "$GITHUB_WORKSPACE/packages" \
  "$GITHUB_WORKSPACE/packages/nemoclaw-openclaw" \
  "$GITHUB_WORKSPACE/packages/nemoclaw-openclaw/plugin"; do
  [[ -d "$plugin_parent" && ! -L "$plugin_parent" ]] \
    || {
      echo "::error::consumer OpenClaw package directory must be a non-symlink directory"
      exit 1
    }
done
plugin_dist="$GITHUB_WORKSPACE/packages/nemoclaw-openclaw/plugin/dist"
[[ ! -e "$plugin_dist" && ! -L "$plugin_dist" ]] \
  || {
    echo "::error::consumer unexpectedly built the OpenClaw package before artifact restore"
    exit 1
  }
...
for boundary in \
  ...
  boundary_path="$restore_dir/dist/lib/shared/$boundary"
...
for boundary in \
  ...
  sandbox-name.cjs \
  snapshot-sanitizer-boundary.cjs; do
  boundary_path="$restore_dir/packages/nemoclaw-openclaw/plugin/dist/shared/$boundary"
  [[ -f "$boundary_path" && ! -L "$boundary_path" && -s "$boundary_path" ]] \
    || {
      echo "::error::restored OpenClaw package shared module is missing or is not a nonempty regular file: $boundary"
      exit 1
    }
done
