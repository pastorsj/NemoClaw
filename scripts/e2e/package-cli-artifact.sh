...
for boundary in \
  ...
  boundary_path="dist/lib/shared/$boundary"
...
for boundary in \
  ...
  sandbox-name.cjs \
  snapshot-sanitizer-boundary.cjs; do
  boundary_path="packages/nemoclaw-openclaw/plugin/dist/shared/$boundary"
  [[ -f "$boundary_path" && ! -L "$boundary_path" && -s "$boundary_path" ]] \
    || {
      echo "::error::candidate OpenClaw package build shared module is missing or is not a nonempty regular file: $boundary"
      exit 1
    }
done
