...
repair_repository() {
  ...
  run_setup_step "Build and type-check the plugin" \
    npm --prefix packages/nemoclaw-openclaw/plugin run build || return 1
...
run_doctor() {
  ...
  check_build_artifact "Plugin build artifacts" "${PLUGIN_BUILD_ARTIFACT}" \
    ...
    "${REPO_ROOT}/packages/nemoclaw-openclaw/plugin/src" \
  ...
  if [ -x "${plugin_tsc}" ]; then
    check_quiet_command_with_heap_hint "Plugin type check" \
      "Run: npm --prefix packages/nemoclaw-openclaw/plugin run build" \
      "${NODE_HEAP_REMEDIATION_PLUGIN}" \
