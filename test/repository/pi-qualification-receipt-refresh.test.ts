...
describe("Pi qualification receipt refresh", () => {
  ...
  beforeEach(() => {
    ...
    fs.mkdirSync(path.join(rootDir, "packages/nemoclaw-pi"), { recursive: true });
    fs.mkdirSync(path.join(rootDir, "ci"), { recursive: true });
    fs.writeFileSync(
      path.join(rootDir, "packages/nemoclaw-pi/Dockerfile"),
      "FROM scratch\nCOPY protected/app /app\n",
    );
    fs.writeFileSync(
      path.join(rootDir, "packages/nemoclaw-pi/Dockerfile.base"),
  ...
  it.each([
    ...
    ["Pi Dockerfile", "packages/nemoclaw-pi/Dockerfile"],
    ["Pi base Dockerfile", "packages/nemoclaw-pi/Dockerfile.base"],
