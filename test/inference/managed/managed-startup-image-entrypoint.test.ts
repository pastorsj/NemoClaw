...
describe("managed startup image hold", () => {
  ...
  it.each(["openclaw", "hermes", "langchain-deepagents-code", "future-harness"] as const)(
    "enters the %s package startup as sandbox after the exact handoff",
    (agent) => {
...
`,
  ...
  it("rejects malformed package IDs before invoking the runtime", () => {
    ...
    expect(result.status).not.toBe(0);
    expect(String(result.stderr ?? "")).toContain("package id is invalid");
  });
