...
const runtimeDirectory = path.join(repoRoot, "packages", "nemoclaw-openclaw", "runtime", "mcporter");
...
describe("mcporter image supply-chain controls", () => {
  ...
  it.each(dockerfiles)("pins and verifies the package in $name", ({ contents }) => {
    ...
    const groupedRuntimeCopy =
      "COPY packages/nemoclaw-openclaw/runtime/mcporter/package.json packages/nemoclaw-openclaw/runtime/mcporter/package-lock.json /usr/local/lib/nemoclaw/mcporter-runtime/";
    const splitRuntimeCopies = [
      ...
      "COPY packages/nemoclaw-openclaw/runtime/mcporter/package-lock.json /usr/local/lib/nemoclaw/mcporter-runtime/package-lock.json",
    ];
    expect(
  ...
  it("verifies the exact committed dependency graph signatures in trusted CI (#8925)", () => {
    ...
    expect(graph).toMatchObject({
      directory: "packages/nemoclaw-openclaw/runtime/mcporter",
