...
`;
  ...
  it("accepts an explicitly selected controller target matrix for the commit under review", () => {
    const workflow = readE2eOperationsWorkflow();
    ...
    try {
      ...
      const controllerResult = spawnSync(
        ...
        {
          encoding: "utf8",
          env: { ...process.env, GITHUB_OUTPUT: output, JOBS: "", TARGETS: targetSelection },
        },
      );
      ...
      expect(
        ...
        JSON.parse(controllerMatrix).map(({ id, runner }: { id: string; runner: string }) => ({
          id,
          runner,
        })),
      );
      expect(actualMatrix).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ id: "ubuntu-repo-cloud-langchain-deepagents-code" }),
          expect.objectContaining({ id: "ubuntu-repo-docker-post-reboot-recovery" }),
        ]),
      );
