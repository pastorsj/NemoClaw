...
`);
    ...
    expect(complete.stdout).toContain(
      "| Repository install onboarding and hosted inference succeed | `ubuntu-repo-cloud-hermes / docker`, `ubuntu-repo-cloud-langchain-deepagents-code / docker`, `ubuntu-repo-cloud-openclaw / docker` | agent runtime and environment or inference endpoint |",
    );
