...
export function createRebuildFlowHarness(overrides: RebuildFlowOverrides = {}): RebuildFlowHarness {
  ...
  vi.spyOn(registry, "getSandbox").mockImplementation(() => {
    ...
    if (sandboxEntryReadCount >= configuredReads.length) return readCurrentSandboxEntry() as never;
  ...
  vi.spyOn(registryPersistence, "load").mockImplementation(() => {
    ...
    const selectedEntry =
      isPreDeleteRead && !recreatedSandboxRegistered ? preDeleteSandboxEntry : currentSandboxEntry;
    return {
      sandboxes: {
  ...
  const onboardSpy = vi
    ...
    .mockImplementation(async (...args: unknown[]) => {
      ...
      Object.assign(currentSandboxEntry, {
  ...
  const establishRestoredSandboxGatewayPairingSpy = vi
    .spyOn(restoreGatewayPairing, "establishRestoredSandboxGatewayPairing")
    .mockImplementation(
      overrides.establishRestoredSandboxGatewayPairing ?? (() => Promise.resolve()),
    );
  ...
  return {
    ...
    establishRestoredSandboxGatewayPairingSpy,
    errorSpy,
