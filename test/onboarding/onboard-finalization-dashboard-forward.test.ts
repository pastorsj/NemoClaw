...
describe("finalization dashboard ForwardTcp launch", () => {
  ...
  it("enables owned-forward reuse only when the package declares it", async () => {
    vi.stubEnv("CHAT_UI_URL", undefined);
    ...
    await expect(
      openClaw.helpers.ensureFinalizationAgentDashboardForward(
        "reonboard-test",
        {
          name: "declared-reuse-agent",
          forwardPort: 18_790,
          dashboard: { reuseOwnedForward: true },
        },
        undefined,
        undefined,
      ),
    ...
    await expect(
      hermes.helpers.ensureFinalizationAgentDashboardForward(
        "reonboard-test",
        {
          name: "ordinary-dashboard-agent",
          forwardPort: 18_790,
          dashboard: { reuseOwnedForward: false },
        },
        undefined,
        undefined,
      ),
