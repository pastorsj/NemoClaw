...
| 02-15 | 10 | COMP-03, COMP-04A | Route, pending policy, provider, and create boundaries require matching package and policy authority | unit/security | `npx vitest run --project cli src/lib/state/registry-route-reservation.test.ts src/lib/onboard/machine/handlers/provider-inference.test.ts src/lib/onboard/sandbox-create/orchestration.test.ts src/lib/onboard/sandbox-create/policy-creation-receipt.test.ts` |
...
| 02-17 | 12 | COMP-04B | Final registration and post-create cancellation preserve exact original-digest resumability | unit/recovery | `npx vitest run --project cli src/lib/onboard/sandbox-registration.test.ts src/lib/onboard/created-sandbox-finalization.test.ts src/lib/onboard/agent-resume-state.test.ts src/lib/onboard/cancel-rollback.test.ts src/lib/onboard/cancel-resume-state.test.ts` |
| 02-18 | 13 | UX-01, COMP-04 | Installer reconciliation converges each independent legacy owner before strict backup or OpenShell mutation without cross-owner adoption | unit/integration/process | Run the focused CLI, integration, and installer-integration commands in Plan 18 |
...
| 02-20 | 15 | COMP-04 | Prepared recovery verifies registry, manifest, object, and definition twice before deletion | unit/recovery | Run the focused prepared-recovery, resume-config, target-image, and target-runtime commands in Plan 20 |
| 02-21 | 16 | COMP-04 | Every downstream rebuild consumer uses the one preflight-pinned definition | unit/recovery | Run the focused flow-helper, post-restore, messaging, GPU, and DCode commands in Plan 21 |
| 02-22 | 17 | TEST-04 | Existing typed base targets install through the public CLI and assert receipt/session/registry equality | e2e-support | `npx vitest run --project e2e-support test/e2e/support/harness-package.test.ts test/e2e/support/e2e-phase-onboarding.test.ts test/e2e/support/e2e-phase-state-validation.test.ts && npm run test:e2e-phases:check` |
