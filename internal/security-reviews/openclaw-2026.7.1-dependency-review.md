...
The committed mcporter lock selects `fast-uri@3.1.6`, `qs@6.15.3`, and
`ip-address@10.3.1`.
...
The September 2 mcporter recheck used the same Node and npm versions. It
reported `1` moderate and `0` high or critical vulnerabilities across `138`
dependencies, verified registry signatures for all `120` installed packages,
and verified attestations for `14` packages. The replacement removes four
high-severity `fast-uri` advisories (`GHSA-5jgf-p345-68v8`,
...
`packages/nemoclaw-openclaw/tests/compat/jaeger-runtime.test.ts` harness runs with
...
`packages/nemoclaw-openclaw/compat/npm-remediation.mts` verifies each original package and
...
`packages/nemoclaw-openclaw/tests/compat/dist-harness.test.ts` materializes the exact public
...
- gateway daemon backend calls ignore the inherited `OPENCLAW_GATEWAY_URL` only
  ...
  `packages/nemoclaw-openclaw/compat/gateway-dialback.mts` is gated to the exact
...
- queued follow-up execution now resolves inbound context before allocating a
  run id; `packages/nemoclaw-openclaw/compat/chat-send.mts` preserves the submitted run id
...
- shared and per-agent SQLite state now run during the required gateway startup
  ...
  `packages/nemoclaw-openclaw/compat/state-permissions.mts` keeps the upstream
...
`packages/nemoclaw-openclaw/compat/device-approval.mts` remains required. Its new
...
`packages/nemoclaw-openclaw/compat/mcp-reliability.mts` is a version-scoped, fail-closed compatibility patch for issue #7958.
...
Coverage: `packages/nemoclaw-openclaw/tests/compat/mcp-reliability.test.ts` pins the compiled preimage, patch idempotence, fail-closed rejection of an unrecognized shape, the classification table, and the retry and diagnostic behavior of the injected runtime.
`packages/nemoclaw-openclaw/tests/helpers/mcp-proof.ts` runs inside the `NEMOCLAW_REAL_OPENCLAW_DIST_HARNESS=1` harness and drives the real patched runtime against a controlled Streamable HTTP MCP server for three scenarios: first exchange resets then succeeds, every exchange resets, and a persistent 401.
...
`packages/nemoclaw-openclaw/compat/transport-diagnostics.mts` is a version-scoped, fail-closed compatibility patch for issue #7957.
...
Managed transport diagnostics remains separate from `packages/nemoclaw-openclaw/compat/mcp-reliability.mts`.
...
The injected helper in `packages/nemoclaw-openclaw/compat/transport-diagnostics.mts` is the shipped runtime source of truth.
`packages/nemoclaw-openclaw/tests/compat/transport-diagnostics.test.ts` executes that exact helper.
...
`packages/nemoclaw-openclaw/compat/mcp-timeout.mts` is a version-scoped, fail-closed compatibility patch.
...
`packages/nemoclaw-openclaw/tests/compat/mcp-timeout.test.ts` executes the injected parser and pins the compiled preimage.
...
Regression coverage lives in `packages/nemoclaw-openclaw/tests/compat/startup-2026-7.test.ts` and
`packages/nemoclaw-openclaw/tests/compat/state-permissions.test.ts`.
...
`packages/nemoclaw-openclaw/tests/config/security-audit.test.ts` locks the generated
suppression scope, and `packages/nemoclaw-openclaw/tests/runtime/audit-suppressions.test.ts`
...
`packages/nemoclaw-openclaw/compat/inference-diagnostics.mts` and
