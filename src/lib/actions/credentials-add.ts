...
export type CredentialsAddDeps = Readonly<{
  ...
  resolveCredentialProviderProfile?: (type: string) => CredentialProviderProfile | null;
}>;
...
export async function runCredentialsAddAction(
  ...
  let profile: CredentialProviderProfile | null;
  try {
    profile = (deps.resolveCredentialProviderProfile ?? resolveCredentialProviderProfile)(type);
  } catch {
    return fail([
      `  Could not resolve provider profile '${type.toLowerCase()}'.`,
