...
type SelectedSkillAgent = Readonly<{
  authority: SandboxCommandAgentAuthority | null;
  binary: string;
  integration: NonNullable<SandboxCommandAgentAuthority["definition"]["skillIntegration"]>;
}>;

function resolveSelectedSkillAgent(sandboxName: string): SelectedSkillAgent | null {
  const entry = registry.getSandbox(sandboxName);
  const authority = entry?.harnessPackage ? captureSandboxCommandAgentAuthority(entry) : null;
  const resolution = agentRuntime.resolveSessionAgentDefinition(
    sandboxName,
    authority?.definition ?? agentRuntime.getSessionAgent(sandboxName),
  );
  ...
  return { authority, binary, integration };
}

function requireCurrentSelectedSkillAgent(sandboxName: string, selected: SelectedSkillAgent): void {
...
export async function removeSandboxSkill(
  ...
  requireCurrentSelectedSkillAgent(sandboxName, selected);
