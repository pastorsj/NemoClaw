...
// @ts-nocheck
//
// Explicit no-receipt quarantine for the legacy Hermes managed-tool host broker.
...
/**
 * Resolve the reviewed Hermes Provider product runtime shipped with NemoClaw.
 *
 * This broker implements a separately qualified provider product, not the
 * receipt-backed harness contract. An installed harness package must never
 * replace executable host code used by this core credential boundary.
 */
function qualifiedHermesProviderPackage() {
...
function captureHermesHostRuntime() {
  const selected = qualifiedHermesProviderPackage();
