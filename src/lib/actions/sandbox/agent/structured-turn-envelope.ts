...
function toolFailureLine(record: Record<string, unknown>): string | null {
  ...
  return `[agent provenance] failed tool result (${toolLabel(record)}): ${detail}`;
}
...
function collectUntrustedChildProvenance(raw: string, docs: unknown[]): string[] {
  ...
  const lines = [
    "[agent provenance] untrusted child result present; verify child-sourced data before treating it as confirmed.",
  ];
  for (const candidate of candidates) {
    const excerpt = untrustedChildExcerpt(candidate);
    if (excerpt) {
      lines.push(`[agent provenance] untrusted child excerpt: ${excerpt}`);
      break;
    }
  }
  return lines;
...
function parseStructuredTurnJsonFrames(raw: string): JsonFrame[] {
  const clean = jsonFrame(raw, 0, raw.length);
  return clean ? [clean] : parseLogPrefixedJsonFrames(raw);
}

/** Parse clean or log-prefixed structured-turn JSON without retrying every candidate suffix. */
export function parseStructuredTurnJsonDocuments(raw: string): unknown[] {
  return parseStructuredTurnJsonFrames(raw).flatMap(({ docs }) => docs);
}

/** Return log or malformed text outside complete structured-turn JSON documents. */
export function structuredTurnUnframedJsonText(raw: string): string {
  const parts: string[] = [];
  let cursor = 0;
  for (const frame of parseStructuredTurnJsonFrames(raw)) {
...
export function structuredTurnProvenanceLines(raw: string): string[] {
  const docs = parseStructuredTurnJsonDocuments(raw);
...
export type StructuredTurnIncompleteSignal = {
...
/** Select a documented local or gateway structured-turn response record. */
export function structuredTurnResponseRecord(doc: unknown): UnknownRecord | null {
...
function agentResponseMetaRecord(doc: unknown): UnknownRecord | null {
  const response = structuredTurnResponseRecord(doc);
 ...
 */
export function structuredTurnIncompleteSignal(raw: string): StructuredTurnIncompleteSignal | null {
  const docs = parseStructuredTurnJsonDocuments(raw);
