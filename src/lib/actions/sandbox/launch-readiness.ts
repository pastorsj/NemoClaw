...
import {
  ...
  isRecordedPortableOpenClawAgent,
...
import {
  ...
  projectLaunchReadinessAgent as projectAgent,
  resolveLaunchHarnessPackageAuthority,
  resolveLaunchReadinessGatewayName as resolveGatewayName,
} from "./launch-readiness/projection";

export { createProbeTimingRecorder, type ProbeTimingRecorder } from "./probe/timing";
export { createBoundLaunchReadinessDeps };
export { buildLaunchReadinessRegistryProjection, launchReadinessDigest };
...
function resolveOpenClawPairingSettlementTarget(
  ...
  if (
    ...
    (entry.agent !== null && !isRecordedPortableOpenClawAgent(entry.agent)) ||
...
export async function settlePortableOpenClawPairing(
  ...
  return withSandboxLock(sandboxName, async () => {
    ...
    if (canIgnorePortableOpenClawReceipt(firstEntry?.agent)) {
    ...
    if (
      ...
      if (!isRecordedPortableOpenClawAgent(firstEntry?.agent)) {
        return incompletePortablePairing("portable-runtime-identity-invalid");
      }
    }
    if (!isRecordedPortableOpenClawAgent(firstEntry?.agent)) {
