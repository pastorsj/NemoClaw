...
function buildConfigHashVerification(errorMessage: string): string {
  return `expected_hash="$(sha256sum openclaw.json fabric.json 2>/dev/null)" && actual_hash="$(cat .config-hash 2>/dev/null)" && [ "$actual_hash" = "$expected_hash" ] || { echo ${shellQuote(errorMessage)} >&2; exit 15; }`;
}
...
function buildOpenClawConfigHashCommandPrefix(configDir: string): string[] {
  return [
    ...
    '[ ! -L "$fabric_file" ] || { echo "refusing symlinked OpenClaw Fabric config: $fabric_file" >&2; exit 19; }',
...
export function buildRefreshMutableOpenClawConfigHashCommand(
  ...
  return [
    ...
    '[ -f "$fabric_file" ] || exit 0',
    'cd "$config_dir" || exit 13',
    `[ "$owner" != "root" ] || { ${buildConfigHashVerification("root-owned OpenClaw config hash does not match its protected files")}; exit 0; }`,
    "sha256sum openclaw.json fabric.json > .config-hash || exit 14",
...
export function buildVerifyMutableOpenClawConfigHashCommand(
  ...
  return [
    ...
    '[ -f "$fabric_file" ] || { echo "OpenClaw Fabric config is not a regular file: $fabric_file" >&2; exit 20; }',
    '[ -f "$hash_file" ] || { echo "OpenClaw config hash is not a regular file: $hash_file" >&2; exit 18; }',
    'cd "$config_dir" || exit 13',
    buildConfigHashVerification("OpenClaw config hash does not match its protected files"),
  ].join("; ");
}
