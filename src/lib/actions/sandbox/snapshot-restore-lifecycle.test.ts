...
describe("runSandboxSnapshot restore: lifecycle and destination safety", () => {
  ...
  it("repairs an empty managed projection but leaves custom-image MCP state to the image (#10756)", async () => {
    ...
    f.getSandboxMock.mockReturnValue(packageManagedSandbox("alpha", DCODE_PACKAGE));
    await runSandboxSnapshot("alpha", { kind: "restore" });
    const prepared = f.prepareInstalledMcpSnapshotRestoreMock.mock.results[0]?.value;
    expect(f.prepareInstalledMcpSnapshotRestoreMock).toHaveBeenCalledWith(
      expect.objectContaining({ name: "alpha", agent: "langchain-deepagents-code" }),
    );
    expect(f.applyInstalledMcpSnapshotRestoreMock).toHaveBeenCalledWith(prepared, {
    ...
    expect(f.restoreSandboxStateMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        agentDefinition: expect.objectContaining({ name: "langchain-deepagents-code" }),
      }),
    );

    f.prepareInstalledMcpSnapshotRestoreMock.mockClear();
    f.applyInstalledMcpSnapshotRestoreMock.mockClear();
    f.getSandboxMock.mockReturnValue({
      ...packageManagedSandbox("alpha", DCODE_PACKAGE),
    ...
    await runSandboxSnapshot("alpha", { kind: "restore" });
    expect(f.prepareInstalledMcpSnapshotRestoreMock).not.toHaveBeenCalled();
    expect(f.applyInstalledMcpSnapshotRestoreMock).not.toHaveBeenCalled();
  ...
  it("repairs the managed Deep Agents MCP projection before restoring snapshot files (#10756)", async () => {
    ...
    f.applyInstalledMcpSnapshotRestoreMock.mockImplementation(() => {
    ...
    expect(f.applyInstalledMcpSnapshotRestoreMock).toHaveBeenCalledWith(
      expect.objectContaining({
        entries: [
  ...
  it("leaves snapshot files untouched when the installed package repair fails (#10756)", async () => {
    f.getLatestBackupMock.mockReturnValue(packageManagedSnapshot(DCODE_PACKAGE));
    f.getSandboxMock.mockReturnValue(packageManagedSandbox("alpha", DCODE_PACKAGE));
    f.applyInstalledMcpSnapshotRestoreMock.mockImplementationOnce(() => {
      throw new Error("package-managed MCP state is unsafe");
    });
    ...
    expect(failure).toMatchObject({
      ...
      lines: [
        ...
        expect.stringContaining("package-managed MCP state is unsafe"),
        expect.stringContaining("Resolve the package-managed MCP state"),
      ],
    });
  ...
  it("clones the selected package identity while preserving its pinned definition", async () => {
  ...
  it.each(["source registry", "target registry", "retained object"] as const)(
    ...
    async (changedAuthority) => {
      ...
      const entries = new Map<string, f.SandboxRecord>([
        ["alpha", packageManagedSandbox("alpha")],
