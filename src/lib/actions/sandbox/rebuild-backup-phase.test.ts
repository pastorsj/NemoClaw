...
import {
  ...
  runRebuildBackupPhase as runRebuildBackupPhaseWithAuthority,
} from "./rebuild-backup-phase";
import { makeRebuildAgentAuthority } from "./rebuild-flow-test-fixtures";

type BackupPhaseTestInput = Omit<
  RebuildBackupPhaseInput,
  "agentAuthority" | "backupRegistryEntry" | "packageAuthority"
> & {
  packageAuthority?: RebuildBackupPhaseInput["packageAuthority"];
};

function runRebuildBackupPhase(
  ...
  return backupStateForRebuild
    ...
    : runRebuildBackupPhaseWithAuthority(authoritativeInput);
}
...
describe("rebuild policy handoff", () => {
  const input = (overrides: Partial<BackupPhaseTestInput> = {}): BackupPhaseTestInput => ({
    sandboxName: "alpha",
...
describe("rebuild backup safety", () => {
  ...
  function customOpenClawInput(overrides: Record<string, unknown> = {}): BackupPhaseTestInput {
    return {
