...
import {
  ...
  LegacyHermesCloneTransactionError,
  prepareLegacyHermesCloneTransaction,
  provisionLegacyHermesCloneTransaction,
  resolveLegacyHermesCloneCredentials,
} from "./legacy-hermes";
...
describe("Hermes managed clone broker transaction", () => {
  ...
  it("stages one secret-free provider-neutral transaction and activates after exact creation", () => {
    ...
    const prepared = prepareLegacyHermesCloneTransaction({
      handoff: preparedHandoff,
      destination: null,
    ...
    const receipt = provisionLegacyHermesCloneTransaction(prepared, {
      ...authority(source),
      environment: environment(),
      runOpenshell: runner.run,
      broker: hostBroker,
  ...
  it("imports the OpenAI profile before provider creation and broker activation (#10155)", () => {
    ...
    provisionLegacyHermesCloneTransaction(prepared, {
      ...authority(source),
      environment: environment(),
      runOpenshell: runner.run,
      broker: hostBroker,
  ...
  it("preserves exact providers when activation outcome is unknown", () => {
    ...
    expect(thrown).toBeInstanceOf(LegacyHermesCloneTransactionError);
    expect((thrown as LegacyHermesCloneTransactionError).cleanupDeferred).toBe(true);
