...
import {
  ...
  LegacyCloneProviderTransactionError,
...
} as const satisfies LegacyCloneProviderBinding;
...
describe("managed clone provider transaction", () => {
  ...
  it("imports the endpointless profile before creating a cloned messaging provider (#9875)", () => {
    ...
    provisionLegacyCloneProviderTransaction(prepared, {
  ...
  it("reuses an exact provider only with exact destination registry ownership", () => {
    ...
    const receipt = provisionLegacyCloneProviderTransaction(prepared, {
    ...
    expect(cleanupLegacyCloneProviderTransaction(receipt, runner.run)).toMatchObject({
      status: "complete",
  ...
  it("revalidates snapshot, source, and destination authority before provider mutation", () => {
    ...
    expect(() =>
      revalidateLegacyCloneMutationAuthority(prepared, {
  ...
  it("rolls back confirmed providers when a later credential disappears", () => {
    ...
    let failure: LegacyCloneProviderTransactionError | null = null;
    try {
      provisionLegacyCloneProviderTransaction(prepared, {
        ...authorityDeps(source),
        environment: { RUNTIME_TOKEN: "test-only-runtime-token" },
      ...
      failure = error as LegacyCloneProviderTransactionError;
    }
    expect(failure).toBeInstanceOf(LegacyCloneProviderTransactionError);
  ...
  it.each([
    ...
    [
      "zero and collision",
      0,
      { ...TOKEN_BINDING, providerType: "other" } satisfies LegacyCloneProviderBinding,
    ],
  ] as const)(
  ...
  it("rejects a cloned or fabricated cleanup receipt", () => {
    ...
    expect(() =>
      cleanupLegacyCloneProviderTransaction(structuredClone(receipt), runner.run),
