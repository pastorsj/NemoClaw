...
import {
  cleanupLegacyCloneProviderTransaction,
  type LegacyCloneProviderBinding,
  type LegacyCloneProviderCleanupResult,
  LEGACY_CLONE_PROVIDER_CREATE_TIMEOUT_MS,
  type LegacyCloneProviderRunner,
  type LegacyCloneProviderTransactionReceipt,
  type PreparedLegacyCloneProviderTransaction,
  prepareLegacyCloneProviderTransaction,
  provisionLegacyCloneProviderTransaction,
  revalidateLegacyCloneMutationAuthority,
...
type LegacyHermesCloneHandoff = Pick<
...
export interface PreparedLegacyHermesCloneTransaction {
  readonly schemaVersion: 1;
  readonly phase: "prepared";
  readonly gatewayProviderName: string;
  readonly inferenceProviderName: string;
  readonly providerTransaction: PreparedLegacyCloneProviderTransaction;
}

export interface LegacyHermesCloneReceipt {
  readonly schemaVersion: 1;
  readonly phase: "activated";
  readonly destinationSandboxName: string;
  readonly providerReceipt: LegacyCloneProviderTransactionReceipt;
}

export class LegacyHermesCloneTransactionError extends Error {
  readonly providerReceipt?: LegacyCloneProviderTransactionReceipt;
  readonly cleanup?: LegacyCloneProviderCleanupResult;
...
function hermesEnabled(handoff: LegacyHermesCloneHandoff): boolean {
 ...
 */
export async function resolveLegacyHermesCloneCredentials(input: {
  readonly handoff: LegacyHermesCloneHandoff;
...
export function prepareLegacyHermesCloneTransaction(input: {
  readonly handoff: LegacyHermesCloneHandoff;
...
}): PreparedLegacyHermesCloneTransaction {
  ...
  const providerTransaction = prepareLegacyCloneProviderTransaction({
...
function ensureLegacyHermesCloneInferenceProviderProfile(
  ...
  throw new LegacyHermesCloneTransactionError(profile.messages.join("\n"));
}

export function provisionLegacyHermesCloneTransaction(
  prepared: PreparedLegacyHermesCloneTransaction,
  input: {
    readonly environment?: NodeJS.ProcessEnv;
    readonly runOpenshell: LegacyCloneProviderRunner;
  ...
  const refreshToken = environment[HERMES_TOOL_GATEWAY_REFRESH_CREDENTIAL_ENV]
    ...
    .trim();
  if (!refreshToken) {
    throw new LegacyHermesCloneTransactionError(
      `credential ${HERMES_TOOL_GATEWAY_REFRESH_CREDENTIAL_ENV} disappeared before broker staging`,
    );
  }

  revalidateLegacyCloneMutationAuthority(prepared.providerTransaction, input);
  ensureLegacyHermesCloneInferenceProviderProfile(input.runOpenshell);
  ...
  let providerReceipt: LegacyCloneProviderTransactionReceipt | undefined;
  try {
    providerReceipt = provisionLegacyCloneProviderTransaction(prepared.providerTransaction, {
      ...input,
      environment,
    ...
    const cleanup = providerReceipt
      ? cleanupLegacyCloneProviderTransaction(providerReceipt, input.runOpenshell)
      : undefined;
    let discarded = false;
    try {
