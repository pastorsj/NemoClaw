...
export const LEGACY_CLONE_PROVIDER_CREATE_TIMEOUT_MS = 30_000;
...
export type LegacyCloneProviderCommandResult = {
...
export type LegacyCloneProviderRunner = (
  args: string[],
  options?: {
...
export interface LegacyCloneProviderBinding {
...
export interface PreparedLegacyCloneProvider {
  readonly binding: LegacyCloneProviderBinding;
  readonly action: "create" | "reuse-destination-owned";
}

export interface PreparedLegacyCloneProviderTransaction {
  ...
  readonly providers: readonly PreparedLegacyCloneProvider[];
}

export interface LegacyCloneProviderOwnershipReceipt {
  readonly binding: LegacyCloneProviderBinding;
  readonly disposition: "created" | "reused-destination-owned";
}

export interface LegacyCloneProviderTransactionReceipt {
  readonly schemaVersion: 1;
  readonly phase: "materialized";
  readonly transactionId: string;
  readonly providerId: string;
  readonly destinationSandboxName: string;
  readonly providers: readonly LegacyCloneProviderOwnershipReceipt[];
}

export type LegacyCloneProviderCleanupOutcome =
  | "already-cleaned"
  | "already-missing"
  | "deleted"
  | "delete-failed"
...
export interface LegacyCloneProviderCleanupResult {
  readonly status: "complete" | "partial";
  readonly providers: readonly {
    readonly providerName: string;
    readonly outcome: LegacyCloneProviderCleanupOutcome;
  }[];
}
...
export class LegacyCloneProviderTransactionError extends Error {
  readonly partialReceipt?: LegacyCloneProviderTransactionReceipt;
  readonly rollback?: LegacyCloneProviderCleanupResult;
...
function fail(message: string, cause?: unknown): never {
  throw new LegacyCloneProviderTransactionError(
    message,
...
function inspectProvider(
  binding: LegacyCloneProviderBinding,
  runOpenshell: LegacyCloneProviderRunner,
): ProviderInspection {
...
function validatedBinding(binding: LegacyCloneProviderBinding): LegacyCloneProviderBinding {
...
function mergeBindings(
  bindings: readonly LegacyCloneProviderBinding[],
): readonly LegacyCloneProviderBinding[] {
  const merged = new Map<string, LegacyCloneProviderBinding>();
...
function applicationBindings(input: {
  ...
  const bindings: LegacyCloneProviderBinding[] = activeMessagingCredentialBindings(
    input.messagingPlan,
    input.sandboxName,
    input.profile.agent,
  ).map((binding) => ({
...
function destinationOwnedBindings(entry: SandboxEntry): readonly LegacyCloneProviderBinding[] {
  const bindings: LegacyCloneProviderBinding[] = activeMessagingCredentialBindings(
...
function sameBinding(left: LegacyCloneProviderBinding, right: LegacyCloneProviderBinding): boolean {
  return (
 ...
 * recovery contract.
 */
export function prepareLegacyCloneProviderTransaction(input: {
  readonly handoff: CloneProviderHandoff;
  readonly destination: SandboxEntry | null;
  readonly additionalBindings?: readonly LegacyCloneProviderBinding[];
  readonly resolveAdditionalDestinationOwnedBindings?: (
    destination: Readonly<SandboxEntry>,
  ) => readonly LegacyCloneProviderBinding[];
  readonly environment?: NodeJS.ProcessEnv;
  readonly runOpenshell: LegacyCloneProviderRunner;
  readonly transactionId?: string;
  ...
  const providers: PreparedLegacyCloneProvider[] = [];
...
export function revalidateLegacyCloneMutationAuthority(
  prepared: PreparedLegacyCloneProviderTransaction,
  input: {
...
function issueReceipt(
  prepared: PreparedLegacyCloneProviderTransaction,
  providers: readonly LegacyCloneProviderOwnershipReceipt[],
 ...
 */
export function provisionLegacyCloneProviderTransaction(
  prepared: PreparedLegacyCloneProviderTransaction,
  input: {
    readonly environment?: NodeJS.ProcessEnv;
    readonly runOpenshell: LegacyCloneProviderRunner;
  ...
  const environment = input.environment ?? process.env;
  const confirmed: LegacyCloneProviderOwnershipReceipt[] = [];
  try {
    // Fence every shared gateway mutation, including provider profile import.
    revalidateLegacyCloneMutationAuthority(prepared, input);
    if (
    ...
    for (const provider of prepared.providers) {
      ...
      let result: LegacyCloneProviderCommandResult;
      try {
        result = input.runOpenshell(
          ...
          ],
          {
            ...
            stdio: ["ignore", "pipe", "pipe"],
            suppressOutput: true,
            timeout: LEGACY_CLONE_PROVIDER_CREATE_TIMEOUT_MS,
          },
        );
      } catch (error) {
    ...
    const rollback = cleanupLegacyCloneProviderTransaction(partialReceipt, input.runOpenshell);
    const detail = cause instanceof Error ? cause.message : String(cause);
    throw new LegacyCloneProviderTransactionError(detail, {
      cause,
      partialReceipt,
      rollback,
    });
 ...
 */
export function cleanupLegacyCloneProviderTransaction(
  receipt: LegacyCloneProviderTransactionReceipt,
  runOpenshell: LegacyCloneProviderRunner,
  ...
  const outcomes: Array<{
    providerName: string;
    outcome: LegacyCloneProviderCleanupOutcome;
  }> = [];
