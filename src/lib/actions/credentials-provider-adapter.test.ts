...
describe("credential actions use typed OpenShell provider results", () => {
  ...
  it("registers both Langfuse keys through the resolved package endpoint profile (#10840)", async () => {
    ...
    const resolveCredentialProviderProfile = vi.fn(() => ({
      profileType: "langfuse-hermes-v1",
      profilePath: "/installed/hermes/provider-profiles/langfuse-hermes-v1.yaml",
    }));
    ...
    const resultPromise = runCredentialsAddAction(
      ...
      { providerAdapter: adapter, resolveCredentialProviderProfile },
    );
    ...
    expect(result.exitCode).toBe(0);
    expect(resolveCredentialProviderProfile).toHaveBeenCalledWith("langfuse-hermes-v1");
  ...
  it("stops before provider import when installed profile authority cannot be resolved", async () => {
    ...
    const result = await runCredentialsAddAction(
      ...
      {
        providerAdapter: adapter,
        resolveCredentialProviderProfile: () => {
    ...
    expect(result).toEqual({
      ...
      successLines: [],
      failureLines: [
        "  Could not resolve provider profile 'custom-profile'.",
        "  Repair or reinstall NemoClaw and its installed harness packages, then retry this command.",
      ],
    });
    expect(JSON.stringify(result)).not.toContain("receipt failed");
