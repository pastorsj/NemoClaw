...
function writeAgentRoot(
  ...
  return writeFile(root, "agents/openclaw/manifest.yaml", `${lines.join("\n")}\n`);
}
...
describe("buildAgentDefinition", () => {
  it("preserves repository definition behavior through the explicit-root builder", () => {
    const manifestPath = path.join(ROOT, "agents/openclaw/manifest.yaml");
    ...
    expect(definition.legacyPaths).toEqual({
      dockerfileBase: path.join(ROOT, "Dockerfile.base"),
      dockerfile: path.join(ROOT, "Dockerfile"),
      startScript: path.join(ROOT, "scripts/nemoclaw-start.sh"),
      policy: path.join(ROOT, "nemoclaw-blueprint/policies/openclaw-sandbox.yaml"),
      plugin: path.join(ROOT, "nemoclaw"),
    });
  });
  ...
  it("resolves ordinary and legacy assets only from the selected package root", () => {
    ...
    const dockerfile = writeFile(fixtureRoot, "agents/openclaw/Dockerfile", "FROM scratch\n");
  ...
  it("keeps same-id definitions from different package roots isolated", () => {
    ...
    writeFile(firstRoot, "agents/openclaw/Dockerfile", "FROM first\n");
    writeFile(secondRoot, "agents/openclaw/Dockerfile", "FROM second\n");
  ...
  it("rejects symbolic links used by ordinary or legacy package assets", () => {
    ...
    fs.symlinkSync(outside, path.join(fixtureRoot, "agents/openclaw/Dockerfile"));
