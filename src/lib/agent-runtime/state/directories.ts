...
import type { AgentStateDirectory, AgentStateDirectoryPath } from "../manifest-types";
import {
...
function assertCanonicalPath(value: string, field: string): void {
  if (value.length === 0) {
  ...
  if (segments.some((segment) => segment.includes("*"))) {
    throw new Error(`Agent manifest field '${field}' must not contain wildcards`);
  }
}
