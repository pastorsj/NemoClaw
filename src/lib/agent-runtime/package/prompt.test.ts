...
describe("promptForHarnessPackage", () => {
  it("offers only uninstalled reviewed packages with the manifest default first", async () => {
    const log = vi.fn();
    ...
    const result = await promptForHarnessPackage({
      inventory: inventory(
        [
          ...
          available("openclaw", "OpenClaw", true),
        ],
  ...
  it("uses the manifest default as the displayed default", async () => {
    const result = await promptForHarnessPackage({
      inventory: inventory([
        available("hermes", "Hermes Agent"),
        available("openclaw", "OpenClaw", true),
  ...
  it.each(["0", "exit", "quit"])("returns an explicit operator exit for %s", async (answer) => {
    const result = await promptForHarnessPackage({
      inventory: inventory([available("openclaw", "OpenClaw", true)]),
      log: vi.fn(),
  ...
  it("reports all installed without opening a prompt", async () => {
    ...
    const result = await promptForHarnessPackage({
      inventory: inventory(
        [available("openclaw", "OpenClaw", true), available("hermes", "Hermes Agent")],
