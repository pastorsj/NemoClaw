...
export function ownSandboxRecreateTransaction(
  ...
  const openingPackageAuthorityFingerprint = fingerprintSandboxRecreateValue({
    harnessPackage: openingSession.harnessPackage,
    harnessPackageMigration: openingSession.harnessPackageMigration,
    checkpointHarnessPackage: openingSession.checkpoint?.harnessPackage ?? null,
  });
    ...
    (current) => {
      if (
        ...
        fingerprintSandboxRecreateValue({
          ...
          checkpointHarnessPackage: current.checkpoint?.harnessPackage ?? null,
        }) !== openingPackageAuthorityFingerprint ||
      ...
      if (
        freshRegistryEntry &&
        (!nullableHarnessPackagesEqual(
          openingSession.harnessPackage,
          freshRegistryEntry.harnessPackage ?? null,
        ) ||
          !isDeepStrictEqual(
            openingSession.harnessPackageMigration,
            freshRegistryEntry.harnessPackageMigration ?? null,
          ))
      ) {
        throw new Error(
          `Cannot start sandbox '${input.sandboxName}' recreate transaction: its registry package authority changed.`,
        );
      }
      if (expectedOld) {
        if (
  ...
  if (
    ...
    fingerprintSandboxRecreateValue({
      harnessPackage: storedSession.harnessPackage,
      harnessPackageMigration: storedSession.harnessPackageMigration,
      checkpointHarnessPackage: storedSession.checkpoint?.harnessPackage ?? null,
    }) !== openingPackageAuthorityFingerprint ||
