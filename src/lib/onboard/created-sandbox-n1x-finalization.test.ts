...
async function completeRegistration(createIntent: CreateIntent): Promise<SandboxEntry> {
  ...
  const [
    ...
    import("../state/onboard-session"),
    import("../state/onboard-checkpoint-migrate"),
    import("../agent/defs"),
  ]);
  ...
  const effectiveAgent = agents.loadAgent("nemocua");
  ...
  session.checkpoint = checkpoints.deriveCheckpointFromSession(session);
