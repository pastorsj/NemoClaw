...
import {
  type LegacyReboundManagedStartupClone,
  rebindLegacyManagedStartupProfileForClone,
} from "../managed-startup/legacy-clone";
...
export interface LegacyManagedCloneSnapshot {
...
export interface LegacyManagedCloneRegistryFields {
...
/**
 * Closed stock-profile clone handoff retained for legacy compatibility.
 * Receipt-backed package profiles are rejected at the snapshot preflight
 * boundary and must never be routed through this harness-specific shape.
 */
export interface PreparedLegacyManagedWorkloadCloneHandoff {
  ...
  readonly rebound: LegacyReboundManagedStartupClone;
  readonly workload: Extract<SandboxWorkloadReceipt, { readonly kind: "managed-image" }>;
  readonly messaging?: SandboxMessagingState;
  readonly registryFields: LegacyManagedCloneRegistryFields;
}

export interface PrepareLegacyManagedCloneInput {
  readonly source: SandboxEntry;
  readonly snapshot: LegacyManagedCloneSnapshot;
  readonly destinationSandboxName: string;
...
export class LegacyManagedCloneError extends Error {
...
function fail(message: string, cause?: unknown): never {
  throw new LegacyManagedCloneError(message, cause === undefined ? undefined : { cause });
}
...
function readSnapshotAuthority(snapshot: LegacyManagedCloneSnapshot): ManagedWorkloadAuthority {
...
function registryFields(
  ...
  source: SandboxEntry,
): LegacyManagedCloneRegistryFields {
  const webSearch =
...
function reboundWorkload(
  ...
  rebound: LegacyReboundManagedStartupClone,
 ...
 */
export function prepareLegacyManagedCloneHandoff(
  input: PrepareLegacyManagedCloneInput,
  ...
  let rebound: LegacyReboundManagedStartupClone;
  try {
    rebound = rebindLegacyManagedStartupProfileForClone({
      sourceSandboxName,
