...
import { LegacyManagedCloneError, prepareLegacyManagedCloneHandoff } from "./legacy-clone";
...
function prepare(
  ...
  return prepareLegacyManagedCloneHandoff({
    source: entry,
    snapshot: {
...
describe("prepareLegacyManagedCloneHandoff", () => {
  it.each([
    ["docker", "openclaw"],
    ["docker", "hermes"],
  ...
  it("fails closed on stale managed authority, provider drift, and missing clone authority", () => {
    ...
    expect(() =>
      prepareLegacyManagedCloneHandoff({
        source: entry,
        snapshot: {
          sandboxName: "source",
          agentType: "openclaw",
    ...
    expect(() => prepare(entry, unauthorized)).toThrow(LegacyManagedCloneError);
  });
