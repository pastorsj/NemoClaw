...
import {
  type LegacyStartupCloneState,
  LegacyStartupCloneError,
  legacyStartupCloneDependencies,
...
function rebind(
  ...
  currentOverrides: Partial<LegacyStartupCloneState> = {},
  names: {
  ...
  return rebindLegacyManagedStartupProfileForClone({
...
describe("rebindLegacyManagedStartupProfileForClone", () => {
  ...
  it("resolves the current Ollama context window when the source provider changed", () => {
    ...
    const resolveContextWindowForModel = vi
      .spyOn(legacyStartupCloneDependencies, "resolveContextWindowForModel")
  ...
  it("fails closed on receipt hash, source messaging identity, and unexpected CA transport", () => {
    ...
    ).toThrow(LegacyStartupCloneError);
