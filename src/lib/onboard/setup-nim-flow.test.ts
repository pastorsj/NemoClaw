...
describe("createSetupNim", () => {
  it.each([
    ["declared package capability", { provider_key_credential_alias: "hosted-inference" }, true],
    ["ambient harness selector", {}, false],
  ] as const)(
    "derives the hosted provider-key alias from %s for receipt-backed onboarding",
    async (_source, inference, expected) => {
      vi.stubEnv("NEMOCLAW_AGENT", "langchain-deepagents-code");
      const stop = new Error("provider discovery reached");
      const getNonInteractiveProvider = vi.fn<SetupNimFlowDeps["getNonInteractiveProvider"]>(() => {
        throw stop;
      });
      const setupNim = createSetupNim(makeDeps({ getNonInteractiveProvider }));
      const agent = { name: "future-harness", inference } as unknown as AgentDefinition;
      ...
      await expect(
        setupNim(
          ...
          undefined,
          undefined,
          packageIdentity("future-harness"),
        ),
      ).rejects.toBe(stop);
      expect(getNonInteractiveProvider).toHaveBeenCalledWith(true, {
        allowHostedInferenceProviderKeyAlias: expected,
      });
    },
  );
  ...
  it("applies a future package API override before route validation and provider probing", async () => {
    const events: string[] = [];
    const agent = {
      name: "future-harness",
      inference: {
    ...
    const routeGuard = vi.fn((route) => {
      events.push(`route:${String(route.preferredInferenceApi)}`);
      return { requiredModel: null, requiredEndpointUrl: null, requiredInferenceApi: null };
    });
    const handleRemoteProviderSelection = vi.fn<SetupNimFlowDeps["handleRemoteProviderSelection"]>(
      async (args, state) => {
    ...
    expect(events[0]).toBe("route:openai-completions");
    expect(events).toContain("provider-probe");
    expect(routeGuard).toHaveBeenCalledWith(
  ...
  it("passes a future package context requirement to Ollama selection", async () => {
    const handleRunningOllamaSelection = vi.fn<SetupNimFlowDeps["handleRunningOllamaSelection"]>(
      async (_gpu, _requestedModel, _recoveredModel, _ollamaRunning, state) => {
        expect(state.ollamaContextWindowFloor).toBe(49_152);
        state.model = "future/model";
        state.provider = "ollama-local";
        state.endpointUrl = "http://127.0.0.1:11434/v1";
        state.credentialEnv = null;
        state.preferredInferenceApi = "openai-completions";
    ...
    const setupNim = createSetupNim(
      makeDeps({
        isNonInteractive: () => true,
        getNonInteractiveProvider: () => "ollama",
        getNonInteractiveModel: () => "future/model",
        detectInferenceProviderHostState: () =>
          makeHostState({ hasOllama: true, ollamaHost: "127.0.0.1", ollamaRunning: true }),
        handleRunningOllamaSelection,
      }),
