...
describe("sandbox create intent machine boundary", () => {
  ...
  it("produces equivalent complete intent for equivalent fresh and resumed selections (#6226)", async () => {
    ...
    for (const variant of variants) {
      ...
      const createIntent = (calls.createSandbox.mock.calls[0] as unknown[] | undefined)?.[15] as unknown as {
        resolved: unknown;
      };
  ...
  it.each([
    ...
    async ({ authoritativeResumeConfig }) => {
      ...
      const createIntent = (calls.createSandbox.mock.calls[0] as unknown[] | undefined)?.[15];
