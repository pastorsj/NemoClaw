...
const { OnboardRestoreSnapshotDriftError } = await import("./session-bootstrap");
      ...
      expect(() =>
        finalizeCreatedSandbox(
          ...
          },
          {
            ...
            restoreRecreatedSandboxState: (name, backup, options) => {
              const authority = captureFixtureRestoreAuthority(backup);
              fs.appendFileSync(path.join(backup, "config.toml"), "\n# changed after selection\n");
              return restoreFixtureState(name, backup, {
                ...options,
                authority,
