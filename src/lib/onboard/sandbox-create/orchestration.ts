...
interface HermesPortableCreateAuthority<T> {
  readonly agent: AgentDefinition;
  readonly gpuAuthority: T;
}

function resolveHermesPortableCreateAuthority<T>(input: {
  readonly enabled: boolean;
  readonly agent: AgentDefinition | null;
  readonly runtimeAuthority: PortableOnboardRuntimeContext["authority"] | null;
...
export function createSandboxWithBaseImageResolution(runtime: SandboxCreateOrchestrationRuntime) {
  ...
  return async function createSandboxWithBaseImageResolution(
    ...
    const hermesPortableAuthority = resolveHermesPortableCreateAuthority({
      enabled: agentCreateInput.hermesPortableLifecycle,
      agent,
      runtimeAuthority: portableRuntimeAuthority,
      createGpuAuthority: (runtimeAuthority) =>
    ...
    const createFlowEnvironment = hermesPortableAuthority?.gpuAuthority.env ?? sandboxEnv;
    const createGpuVerifier =
      hermesPortableAuthority?.gpuAuthority.verify ?? verifyDirectSandboxGpu;
    ...
    const runCreateFlow = async (
      ...
      >({
        ...
        persistCreateIdentity: (_identity, _exactIdentity, boundary) => {
          ...
          const operation = `record verified create checkpoint for sandbox '${sandboxName}'`;
          const harnessPackageAuthority = revalidateHarnessPackageAuthority(operation);
