...
export class MessagingProviderApplyError extends Error {
  ...
  constructor(input: {
    ...
    this.providerIds = Object.freeze({ ...(input.providerIds ?? {}) });
    this.attachedProviderNames = uniqueStrings(input.attachedProviderNames ?? []);
  }
}
...
export async function applyCredentialsAtOpenShell(
  ...
  for (const definition of definitions) {
    ...
    if (
      observed.ok &&
      definition.expectedProviderId &&
      observed.value.id !== definition.expectedProviderId
    ) {
      throw stableIdentityConflict(definition);
    }
    if (
      options.requireStableProviderIdentity &&
      state === "exact" &&
      !definition.expectedProviderId
    ) {
      throw new MessagingProviderApplyError({
        message: `Messaging provider '${definition.providerName}' exists without a NemoClaw ownership receipt.`,
    ...
    if (
      ...
      throw new MessagingProviderApplyError({
        message: `Messaging provider '${definition.providerName}' is missing; refusing to create a replacement identity.`,
        bindingConflict: true,
      });
    }
    if (state === "exact") {
      recordProviderIdentity(definition, observed, providerIds, options);
    }
  ...
  for (const definition of definitions) {
    ...
    if (state === "collision") {
      if (options.requireStableProviderIdentity && definition.allowProviderReplacement !== true) {
        throw new MessagingProviderApplyError({
          message: `Messaging provider '${definition.providerName}' is not owned for replacement.`,
          bindingConflict: true,
        });
      }
      try {
        await deleteProviderForReplacement(definition, options, providerAdapter);
      } catch (error) {
    ...
    if (
      classifyProviderDefinition(verification, definition) !== "exact" ||
      (definition.expectedProviderId &&
        action === "update" &&
        (!verification.ok || verification.value.id !== definition.expectedProviderId))
    ) {
      throw withMutationEvidence(
    ...
    try {
      recordProviderIdentity(definition, verification, providerIds, options);
    } catch (error) {
      throw withMutationEvidence(
        error,
        mutatedProviderNames,
  ...
  try {
    const refreshedProviderNames = await configureRefreshes(refreshes, options, providerAdapter);
    for (const providerName of refreshedProviderNames) {
      if (!mutatedProviderNames.includes(providerName)) mutatedProviderNames.push(providerName);
    }
  } catch (error) {
    throw withMutationEvidence(
      error,
  ...
  try {
    await confirmProviderIdentities(definitions, providerIds, options, providerAdapter);
  } catch (error) {
    throw withMutationEvidence(
      error,
      mutatedProviderNames,
  ...
  let attachedProviderNames: readonly string[] = [];
  if (options.attachToSandbox) {
    try {
      attachedProviderNames = await attachProviders(
        providerNames,
...
export async function cleanupProvidersAtOpenShell(
  ...
  for (const providerName of uniqueStrings(providerNames)) {
    ...
    const initialProviderIdentity = options.expectedProviderIds
      ? await inspectCleanupProviderIdentity(providerName, options, target)
      : "exact";
    if (initialProviderIdentity === "missing") {
      absentProviderNames.push(providerName);
      continue;
    }
    if (initialProviderIdentity !== "exact") {
      residualProviders.push({
        providerName,
        error: redactedProviderError(initialProviderIdentity),
      });
      continue;
    }
    if (detachOnly.has(providerName)) {
      if (allowed.size === 0) {
        residualProviders.push({
          providerName,
          error: {
            kind: "validation",
            message: "No sandbox attachment is authorized for provider cleanup.",
          },
        });
        continue;
      }
      let detachOnlyError: OpenShellProviderError | null = null;
      for (const sandboxName of allowed) {
        const identityError = cleanupIdentityError(options, providerName, "detach");
        if (identityError) {
          detachOnlyError = identityError;
          break;
        }
        const identity = await inspectCleanupProviderIdentity(providerName, options, target);
        if (identity !== "exact") {
          detachOnlyError =
            identity === "missing"
              ? { kind: "command", reason: "not_found", message: "Provider is absent." }
              : identity;
          break;
        }
        ...
        if (!detached.ok) {
          detachOnlyError = detached.error;
          break;
        }
      ...
      if (detachOnlyError) {
        residualProviders.push({
          providerName,
          error: redactedProviderError(detachOnlyError),
        });
      }
      continue;
    }
    ...
    const retryProviderIdentity = options.expectedProviderIds
      ? await inspectCleanupProviderIdentity(providerName, options, target)
      : "exact";
    if (retryProviderIdentity === "missing") {
      detachedAttachments.push(...providerDetachedAttachments);
      absentProviderNames.push(providerName);
      continue;
    }
    if (retryProviderIdentity !== "exact") {
      detachedAttachments.push(...providerDetachedAttachments);
      residualProviders.push({
        providerName,
        error: await restoreCleanupAttachments(
...
async function restoreCleanupAttachments(
  ...
  options: Pick<
    MessagingProviderCleanupOptions,
    "expectedProviderIds" | "revalidateSandboxIdentity"
  >,
  ...
  for (const { sandboxName } of [...attachments].reverse()) {
    ...
    if (options.expectedProviderIds) {
      const identity = await inspectCleanupProviderIdentity(
        providerName,
        {
          providerAdapter,
...
async function inspectCleanupProviderIdentity(
  providerName: string,
  options: MessagingProviderCleanupOptions,
  target: OpenShellGatewayTarget,
): Promise<"exact" | "missing" | OpenShellProviderError> {
  const expectedIds = options.expectedProviderIds;
  const expectedProviderId = expectedIds?.[providerName];
  if (expectedIds && !expectedProviderId) {
    return {
      kind: "validation",
      message: `Messaging provider '${providerName}' has no stable cleanup authority.`,
    };
  }
  let observed: OpenShellProviderResult<OpenShellProviderMetadata>;
  try {
    observed = await options.providerAdapter.getProvider({ target, providerName });
  } catch {
    return {
      kind: "transport",
      reason: "connection_loss",
      message: `Messaging provider '${providerName}' identity could not be inspected.`,
    };
  }
  if (!observed.ok) {
    return isNotFound(observed) ? "missing" : observed.error;
  }
  if (expectedProviderId && observed.value.id !== expectedProviderId) {
    return {
      kind: "transport",
...
async function prepareProfiles(
  ...
  const profiles = new Map<
    string,
    NonNullable<MessagingCredentialProviderEphemeralInput["profile"]>
  >();
  for (const { profile } of definitions) {
    if (!profile) continue;
    const existing = profiles.get(profile.profileType);
    if (
      existing &&
      (existing.profilePath !== profile.profilePath ||
        existing.profileSource !== profile.profileSource)
    ) {
      throw new MessagingProviderApplyError({
        message: `Messaging provider profile '${profile.profileType}' has conflicting definitions.`,
      });
    }
    profiles.set(profile.profileType, profile);
  }
  for (const [profileType, profile] of profiles) {
    const imported = await importProviderProfile(profile, target, providerAdapter);
    if (!imported.ok) {
      throw new MessagingProviderApplyError({
...
async function importProviderProfile(
  profile: NonNullable<MessagingCredentialProviderEphemeralInput["profile"]>,
  target: OpenShellGatewayTarget,
  providerAdapter: OpenShellProviderAdapter,
) {
  if (profile.profileSource === undefined) {
    return providerAdapter.importProviderProfile({ target, profilePath: profile.profilePath });
  }
  if (Buffer.byteLength(profile.profileSource, "utf8") > 64 * 1024) {
    return {
      ok: false as const,
      error: {
  ...
  const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-provider-profile-"));
  const temporaryProfilePath = path.join(temporaryDirectory, "profile.yaml");
  try {
    fs.writeFileSync(temporaryProfilePath, profile.profileSource, {
      encoding: "utf8",
      flag: "wx",
      mode: 0o600,
    });
    return await providerAdapter.importProviderProfile({
      target,
...
function stableIdentityConflict(
  definition: MessagingCredentialProviderEphemeralInput,
): MessagingProviderApplyError {
  return new MessagingProviderApplyError({
    message: `Messaging provider '${definition.providerName}' no longer matches its recorded stable identity.`,
    bindingConflict: true,
  });
}

function recordProviderIdentity(
  definition: MessagingCredentialProviderEphemeralInput,
  result: OpenShellProviderResult<OpenShellProviderMetadata>,
  providerIds: Record<string, string>,
  options: Pick<MessagingCredentialApplyOptions, "requireStableProviderIdentity">,
): void {
  if (!result.ok || !result.value.id) {
    if (options.requireStableProviderIdentity) {
      throw new MessagingProviderApplyError({
        message: `Messaging provider '${definition.providerName}' did not expose a stable identity.`,
      });
    }
    return;
  }
  if (definition.expectedProviderId && result.value.id !== definition.expectedProviderId) {
    throw stableIdentityConflict(definition);
  }
  providerIds[definition.providerName] = result.value.id;
}

async function confirmProviderIdentities(
  definitions: readonly MessagingCredentialProviderEphemeralInput[],
  providerIds: Readonly<Record<string, string>>,
  options: Pick<
    MessagingCredentialApplyOptions,
    "requireStableProviderIdentity" | "revalidateSandboxIdentity" | "target"
  >,
  providerAdapter: OpenShellProviderAdapter,
): Promise<void> {
  const target = options.target ?? selectedOpenShellGateway();
  ...
  for (const [providerName, providerId] of Object.entries(providerIds)) {
    const definition = definitionsByName.get(providerName);
    if (!definition) continue;
    options.revalidateSandboxIdentity?.(
      `confirm messaging provider ${JSON.stringify(providerName)} before attachment`,
    );
    const observed = await providerAdapter.getProvider({ target, providerName });
    if (
      classifyProviderDefinition(observed, definition) !== "exact" ||
      !observed.ok ||
      observed.value.id !== providerId
    ) {
      throw new MessagingProviderApplyError({
        message: `Messaging provider '${providerName}' changed before attachment.`,
      });
    }
    options.revalidateSandboxIdentity?.(
      `confirm messaging provider ${JSON.stringify(providerName)} identity observation`,
    );
  }
  if (
    options.requireStableProviderIdentity &&
    definitions.some(
      (definition) =>
        !providerIds[definition.providerName] &&
        definition.credentials.some(({ value }) => Boolean(value)),
    )
  ) {
    throw new MessagingProviderApplyError({
      message: "OpenShell did not expose stable identities for every applied messaging provider.",
    });
  }
}
...
async function deleteProviderForReplacement(
  ...
  const target = options.target ?? selectedOpenShellGateway();
  await requireExpectedProviderIdentity(definition, target, providerAdapter);
...
async function requireExpectedProviderIdentity(
  definition: MessagingCredentialProviderEphemeralInput,
  target: OpenShellGatewayTarget,
  providerAdapter: OpenShellProviderAdapter,
): Promise<void> {
  if (!definition.expectedProviderId) return;
  const observed = await providerAdapter.getProvider({
    target,
    providerName: definition.providerName,
  });
  if (!observed.ok || observed.value.id !== definition.expectedProviderId) {
    throw stableIdentityConflict(definition);
  }
}
...
function redactedMutationFailure(error: unknown): MessagingProviderApplyError {
  ...
  return new MessagingProviderApplyError({
    ...
    attachedProviderNames: existing?.attachedProviderNames,
  });
}
...
async function configureRefreshes(
  ...
  const refreshedProviderNames: string[] = [];
  for (const refresh of refreshes) {
    options.revalidateSandboxIdentity?.(
    ...
    if (status === "refreshed") {
      refreshedProviderNames.push(refresh.providerName);
      continue;
    }
    if (observationError) {
      throw new MessagingProviderApplyError({
  ...
  return Object.freeze(uniqueStrings(refreshedProviderNames));
}
...
async function attachProviders(
  ...
  const alreadyAttached = new Set(options.alreadyAttachedProviderNames ?? []);
  for (const providerName of providerNames) {
    if (alreadyAttached.has(providerName)) continue;
    try {
      options.revalidateSandboxIdentity?.(
      ...
      throw withMutationEvidence(
        error,
        [...attachedProviderNames, providerName],
        [],
        [],
        {},
        attachedProviderNames,
  ...
  return Object.freeze([...attachedProviderNames]);
}
...
function withMutationEvidence(
  ...
  const existingProviderIds = error instanceof MessagingProviderApplyError ? error.providerIds : {};
  const existingAttached =
    error instanceof MessagingProviderApplyError ? error.attachedProviderNames : [];
  return new MessagingProviderApplyError({
    message,
    bindingConflict: isMessagingProviderBindingConflict(error),
    ...
    providerIds: { ...existingProviderIds, ...providerIds },
    attachedProviderNames: [...existingAttached, ...attachedProviderNames],
    cause:
