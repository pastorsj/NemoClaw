...
describe("messaging OpenShell provider application", () => {
  ...
  it.each([
    ...
    ["credential keys", { credentialKeys: ["TELEGRAM_BOT_TOKEN", "UNEXPECTED_TOKEN"] }],
  ])(
  ...
  it("refuses to recreate a missing receipt-owned provider identity", async () => {
    const expected = definition({ expectedProviderId: "provider-id-a" });
    const adapter = providerAdapter();

    await expect(
      applyCredentialsAtOpenShell(plan, {
        providerAdapter: adapter,
        target,
        definitions: [expected],
      ...
      message: expect.stringContaining("missing; refusing to create a replacement identity"),
    });
    expect(adapter.createProvider).not.toHaveBeenCalled();
    expect(adapter.updateProvider).not.toHaveBeenCalled();
  });

  it("refuses to update a provider whose stable identity differs from its receipt", async () => {
    const expected = definition({ expectedProviderId: "provider-id-a" });
    const adapter = providerAdapter({
      getProvider: vi.fn<OpenShellProviderAdapter["getProvider"]>().mockResolvedValue({
        ok: true,
        value: { ...metadata(expected), id: "provider-id-b" },
      }),
    });
    ...
    await expect(
      ...
      code: "NEMOCLAW_MESSAGING_PROVIDER_BINDING_CONFLICT",
      message: expect.stringContaining("stable identity"),
    });
    expect(adapter.createProvider).not.toHaveBeenCalled();
    expect(adapter.updateProvider).not.toHaveBeenCalled();
  });
      ...
      tokenDefs: [
        {
          ...
          messagingProviderProfile: profile,
        },
      ],
      root: "/repo",
      agent: "openclaw",
  ...
  it("refuses cleanup when a provider differs from its receipt identity", async () => {
    const providerName = "alpha-telegram-bridge";
    const adapter = providerAdapter({
      getProvider: vi.fn<OpenShellProviderAdapter["getProvider"]>().mockResolvedValue({
        ok: true,
    ...
    const result = await cleanupProvidersAtOpenShell([providerName], {
      ...
      target,
      allowedSandboxes: ["alpha"],
      expectedProviderIds: { [providerName]: "provider-id-a" },
    });
