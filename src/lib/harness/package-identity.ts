...
export function parseHarnessPackageIdentity(value: unknown): HarnessPackageIdentity {
  const identity = parseReceiptHarnessPackageIdentity(value);
  const aliasTarget = AGENT_ALIASES[identity.id];
  if (aliasTarget !== undefined && aliasTarget !== identity.id) {
    throw new Error("Harness package identity id must not use an agent alias");
  }
