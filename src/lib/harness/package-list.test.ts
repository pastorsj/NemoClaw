...
function packageIdentity(
  ...
  return { kind: "agent-runtime", id, packageVersion, contractVersion: 1, contentDigest };
}
...
describe("harness package inventory presentation", () => {
  it("reports no installed harnesses while retaining the reviewed available inventory", () => {
    ...
    expect(text).toBe(
      [
        ...
        "",
        "Available",
        "  openclaw | OpenClaw | adapter 0.1.0 | contract 1 | digest aaaaaaaaaaaa… | not installed",
      ].join("\n"),
    );
  ...
  it("rejects an alias in a machine installed identity", () => {
    const view = createView({
      installed: [healthyPackage("openclaw", "OpenClaw", "0.1.0", OPENCLAW_DIGEST, true)],
      available: [OPENCLAW_AVAILABLE],
    });
    const aliased = renderHarnessInventoryJson(view).replaceAll("openclaw", "deepagents");

    expect(() => parseHarnessInventoryJson(aliased)).toThrow(/must not use an agent alias/);
  });
