...
function writePackage(
  ...
  const manifestPath = path.join(
    root,
    ...(options.manifestRelativePath ?? "agents/openclaw/manifest.yaml").split("/"),
  );
...
describe("parseHarnessPackageManifest", () => {
  it("returns the exact package envelope and manifest data without building an agent", () => {
    ...
    expect(result.manifestPath).toBe(path.join(root, "agents", "openclaw", "manifest.yaml"));
  ...
  it.each([
    ...
    ["contract version", { contractVersion: 2 }, "contractVersion must be 1"],
  ...
  it.each(['"id":"openclaw","id":"hermes"', '"\\u0069d":"openclaw","id":"hermes"'])(
    ...
    (duplicateMembers) => {
      const root = makeRoot();
      writePackage(root, {
        metadata: `{"schemaVersion":1,"kind":"agent-runtime",${duplicateMembers},"displayName":"OpenClaw","packageVersion":"1.2.3","contractVersion":1,"manifest":"agents/openclaw/manifest.yaml"}`,
      });
  ...
  it.each([
    ...
    "./agents/manifest.yaml",
    "agents\\openclaw\\manifest.yaml",
    "C:/agents/openclaw/manifest.yaml",
    "agents/openclaw/manifest.yaml:stream",
  ...
  it.each(["metadata", "manifest"])("rejects a linked %s file", (target) => {
    ...
    const targetPath =
      target === "metadata"
        ? path.join(root, HARNESS_PACKAGE_METADATA_FILE)
        : path.join(root, "agents", "openclaw", "manifest.yaml");
