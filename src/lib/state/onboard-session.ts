...
export interface SessionCancellationRecovery {
  ...
  readonly policyCreationReceipt:
    | NonNullable<RetainedSandboxRecoveryRecord["policyCreationReceipt"]>
    | null;
...
function hasValidPresentHarnessAgent(source: UnknownRecord): boolean {
  if (!hasOwn(source, "agent") || source.agent === null) return true;
  try {
    parseHarnessPackageId(source.agent);
    return true;
  } catch {
    return false;
  }
...
export function readOnboardSessionState(): OnboardSessionReadResult {
  ...
  try {
    ...
    if (
      !session ||
      !isObject(parsed) ||
      hasInvalidSessionHarnessPackage(session) ||
    ...
    if (
      ...
      try {
        writeRetainedSandboxRecovery(RETAINED_SANDBOX_RECOVERY_FILE, {
          ...
          harnessPackage: requireRetainedRecoveryPackage(current),
